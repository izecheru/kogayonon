...
public:
   ...
   */
  auto getData( resources::Mesh* pModel ) -> InstanceData*;
  ...
  void addDirectionalLight( entt::entity entityId, const core::DirectionalLightComponent& other );
  ...
  auto getPointLight( uint32_t index ) -> resources::PointLight&;
  auto getDirectionalLight( uint32_t index = 0 ) -> resources::DirectionalLight&;

  auto getLightCount( const resources::LightType& type ) -> uint32_t;*/
...
private:
  ...
  std::unordered_map<resources::Mesh*, std::unique_ptr<InstanceData>> m_instances;
};
} // namespace core