  ...
  for ( auto i = 0u; i < m_model.meshes_count; ++i )
  {
    ...
    for ( auto j = 0u; j < mesh->primitives_count; ++j )
    {
      ...
      if ( primitive->material >= 0 && primitive->material < m_model.materials_count )
      {
        ...
        mat.diffuseTextureIndex = imageIndexFromTexture( material->pbr_metallic_roughness.base_color_texture.index );
      ...
      if ( primitive->indices != -1 )
      {
        ...
        if ( accessor->component_type == TG3_COMPONENT_TYPE_UNSIGNED_INT )
        {
          const uint32_t* buffData =
            reinterpret_cast<const uint32_t*>( buffer->data.data + bufferView->byte_offset + accessor->byte_offset );
        ...
        {
          const uint16_t* buffData =
            reinterpret_cast<const uint16_t*>( buffer->data.data + bufferView->byte_offset + accessor->byte_offset );
