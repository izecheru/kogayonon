...
void ModelLoader::assignModelMeshes(const cgltf_data* data, std::vector<Mesh>& meshes)
{
  ...
  for (size_t node_index = 0; node_index < data->nodes_count; node_index++)
  {
    ...
    {
      meshes.emplace_back(std::move(final_vertices), std::move(indices), std::move(texture_paths));
    }
...
void ModelLoader::parseVertices(cgltf_primitive& primitive, std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals,
