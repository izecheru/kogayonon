...
namespace kogayonon
{
  ...
  {
  public:
    template <typename S> bool openFile(const std::string& path, S& file_stream, FileMode mode)
    {
    ...
    template <typename S> bool closeFile(S&& file_stream)
    {
    ...
    template <typename V> bool serializeRaw(const V* data, size_t size, std::ofstream& out)
    {
      out.write(reinterpret_cast<const char*>(data), size);
      return isGood(out);
    }

    template <typename V> bool deserializeRaw(V* data, size_t size, std::ifstream& in)
    {
      in.read(reinterpret_cast<char*>(data), size);
      return isGood(in);
    }

    template <typename V> bool serializeVar(V& var, std::ofstream& out)
    {
      out.write(reinterpret_cast<const char*>(&var), sizeof(V));
      return isGood(out);
    }

    template <typename V> bool deserializeVar(V& var, std::ifstream& in)
    {
    ...
    template <typename S> inline bool isGood(S& file_stream)
    {
