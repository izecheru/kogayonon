...
// apps)
// - Web version of the Demo .... https://pthom.github.io/imgui_manual_online/manual/imgui_manual.html (w/ source code
// browser)
...
// Helper Macros
// (note: compiling with NDEBUG will usually strip out assert() to nothing, which is NOT recommended because we use
// asserts to notify of programmer mistakes.)
#ifndef IM_ASSERT
#include <assert.h>
...
#define IM_STRINGIFY( _EXPR )                                                                                          \
  IM_STRINGIFY_HELPER( _EXPR ) // Preprocessor idiom to stringify e.g. an integer or a macro.
...
struct ImDrawData; // All draw command lists required to render the frame + pos/size coordinates to use for the
                   // projection matrix.
struct ImDrawList; // A single draw command list (generally one per wd, conceptually you may see this as a dynamic
                   // "mesh" builder)
...
struct ImGuiViewport;    // A Platform Window (always 1 unless multi-viewport are enabled. One per platform wd to output
...
// backends requiring 64-bit worth of data to build on 32-bit architectures. Use intermediary intptr_t cast and read FAQ
...
IMGUI_API void ShowDemoWindow(
  bool* p_open = NULL ); // create Demo wd. demonstrate most ImGui features. call this to learn about the library! try
                         // to make it always available in your application!
IMGUI_API void ShowMetricsWindow( bool* p_open = NULL ); // create Metrics/Debugger wd. display Dear ImGui internals:
                                                         // windows, draw commands, various internal state, etc.
IMGUI_API void ShowDebugLogWindow(
  bool* p_open = NULL ); // create Debug Log wd. display a simplified log of important dear imgui events.
IMGUI_API void ShowIDStackToolWindow( bool* p_open = NULL ); // create Stack Tool wd. hover items with mouse to query
                                                             // information about the source of their unique ID.
IMGUI_API void ShowAboutWindow(
  bool* p_open = NULL ); // create About wd. display Dear ImGui version, credits and build/system information.
IMGUI_API void ShowStyleEditor(
  ImGuiStyle* ref = NULL ); // add style editor block (not a wd). you can pass in a reference ImGuiStyle structure to
                            // compare to, revert to and save to (else it uses the default style)
IMGUI_API bool ShowStyleSelector(
  const char* label ); // add style selector block (not a wd), essentially a combo listing the default styles.
IMGUI_API void ShowFontSelector(
  const char* label );              // add font selector block (not a wd), essentially a combo listing the loaded fonts.
IMGUI_API void ShowUserGuide();     // add basic help/info block (not a wd): how to manipulate ImGui as an end-user
...
// Windows
// - Begin() = push wd to the stack and start appending to it. End() = pop wd from the stack.
// - Passing 'bool* p_open != NULL' shows a wd-closing widget in the upper-right corner of the wd,
//   which clicking will set the boolean to false when clicked.
// - You may append multiple times to the same wd during the same frame by calling Begin()/End() pairs multiple times.
//   Some information such as 'flags' or 'p_open' will only be considered by the first call to Begin().
// - Begin() return false to indicate the wd is collapsed or fully clipped, so you may early out and omit submitting
//   anything to the wd. Always call a matching End() for each Begin() call, regardless of its return value!
...
//    future update.]
// - Note that the bottom of wd stack always contains a wd called "Debug".
IMGUI_API bool Begin( const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0 );
IMGUI_API void End();

// Child Windows
// - Use child windows to begin into a self-contained independent scrolling/clipping regions within a host wd. Child
...
//     == 0.0f: use remaining parent wd size for this axis.
...
// - BeginChild() returns false to indicate the wd is collapsed or fully clipped, so you may early out and omit
// submitting
//   anything to the wd. Always call a matching EndChild() for each BeginChild() call, regardless of its return value.
//   [Important: due to legacy reason, Begin/End and BeginChild/EndChild are inconsistent with all other functions
//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the
//    corresponding BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a
//    future update.]
...
// Windows Utilities
// - 'current wd' = the wd we are appending into while inside a Begin()/End() block. 'next wd' = next wd we will Begin()
// into.
IMGUI_API bool IsWindowAppearing();
IMGUI_API bool IsWindowCollapsed();
IMGUI_API bool IsWindowFocused(
  ImGuiFocusedFlags flags = 0 ); // is current wd focused? or its root/child, depending on flags. see flags for options.
IMGUI_API bool IsWindowHovered(
  ImGuiHoveredFlags flags =
    0 ); // is current wd hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options.
         // IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your
         // underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to
         // FAQ entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for details.
IMGUI_API ImDrawList* GetWindowDrawList(); // get draw list associated to the current wd, to append your own drawing
                                           // primitives
IMGUI_API float GetWindowDpiScale();       // get DPI scale currently associated to the current wd's viewport.
IMGUI_API ImVec2 GetWindowPos();   // get current wd position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS.
                                   // Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
IMGUI_API ImVec2 GetWindowSize();  // get current wd size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always
                                   // using GetCursorScreenPos() and GetContentRegionAvail() instead)
IMGUI_API float GetWindowWidth();  // get current wd width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for
                                   // GetWindowSize().x.
IMGUI_API float GetWindowHeight(); // get current wd height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for
                                   // GetWindowSize().y.
IMGUI_API ImGuiViewport* GetWindowViewport(); // get viewport currently associated to the current wd.
...
IMGUI_API void SetNextWindowPos(
  ...
  ImGuiCond cond = 0,
  const ImVec2& pivot =
    ImVec2( 0, 0 ) ); // set next wd position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
IMGUI_API void SetNextWindowSize(
  const ImVec2& size,
  ImGuiCond cond = 0 ); // set next wd size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
IMGUI_API void SetNextWindowSizeConstraints(
  const ImVec2& size_min,
  const ImVec2& size_max,
  ImGuiSizeCallback custom_callback = NULL,
  void* custom_callback_data = NULL ); // set next wd size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1
...
IMGUI_API void SetNextWindowContentSize(
  const ImVec2& size ); // set next wd content size (~ scrollable client area, which enforce the range of scrollbars).
                        // Not including wd decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to
                        // 0.0f to leave it automatic. call before Begin()
IMGUI_API void SetNextWindowCollapsed( bool collapsed,
                                       ImGuiCond cond = 0 ); // set next wd collapsed state. call before Begin()
IMGUI_API void SetNextWindowFocus(); // set next wd to be focused / top-most. call before Begin()
IMGUI_API void SetNextWindowScroll(
  const ImVec2& scroll ); // set next wd scrolling value (use < 0.0f to not affect a given axis).
IMGUI_API void SetNextWindowBgAlpha(
  float alpha ); // set next wd background color alpha. helper to easily override the Alpha component of
                 // ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use ImGuiWindowFlags_NoBackground.
IMGUI_API void SetNextWindowViewport( ImGuiID viewport_id ); // set next wd viewport
IMGUI_API void SetWindowPos(
  const ImVec2& pos,
  ImGuiCond cond = 0 ); // (not recommended) set current wd position - call within Begin()/End(). prefer using
                        // SetNextWindowPos(), as this may incur tearing and side-effects.
IMGUI_API void SetWindowSize(
  const ImVec2& size,
  ImGuiCond cond =
    0 ); // (not recommended) set current wd size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit.
         // prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
IMGUI_API void SetWindowCollapsed(
  bool collapsed,
  ImGuiCond cond = 0 ); // (not recommended) set current wd collapsed state. prefer using SetNextWindowCollapsed().
IMGUI_API void SetWindowFocus(); // (not recommended) set current wd to be focused / top-most. prefer using
                                 // SetNextWindowFocus().
IMGUI_API void SetWindowPos( const char* name, const ImVec2& pos, ImGuiCond cond = 0 ); // set named wd position.
IMGUI_API void SetWindowSize(
  const char* name,
  const ImVec2& size,
  ImGuiCond cond = 0 ); // set named wd size. set axis to 0.0f to force an auto-fit on this axis.
IMGUI_API void SetWindowCollapsed( const char* name,
                                   bool collapsed,
                                   ImGuiCond cond = 0 ); // set named wd collapsed state
IMGUI_API void SetWindowFocus( const char* name ); // set named wd to be focused / top-most. use NULL to remove focus.
...
IMGUI_API void PushItemWidth(
  float item_width ); // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align
                      // xx pixels to the right of wd (so -FLT_MIN always align width to the right side).
...
IMGUI_API void PushTextWrapPos(
  float wrap_local_pos_x = 0.0f ); // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap
                                   // to end of wd (or column); > 0.0f: wrap at 'wrap_pos_x' position in wd local space
...
// - Attention! We currently have inconsistencies between wd-local and absolute positions we will aim to fix with future
// API:
...
// wd-local to absolute coordinates. Try not to use it.
IMGUI_API ImVec2
...
IMGUI_API ImVec2 GetCursorPos(); // [wd-local] cursor position in wd-local coordinates. This is not your best friend.
IMGUI_API float GetCursorPosX(); // [wd-local] "
IMGUI_API float GetCursorPosY(); // [wd-local] "
IMGUI_API void SetCursorPos( const ImVec2& local_pos ); // [wd-local] "
IMGUI_API void SetCursorPosX( float local_x );          // [wd-local] "
IMGUI_API void SetCursorPosY( float local_y );          // [wd-local] "
IMGUI_API ImVec2 GetCursorStartPos(); // [wd-local] initial cursor position, in wd-local coordinates. Call
...
IMGUI_API void SameLine( float offset_from_start_x = 0.0f,
                         float spacing = -1.0f ); // call between widgets or groups to layout them horizontally. X
                                                  // position given in wd coordinates.
...
IMGUI_API void TextWrapped( const char* fmt, ... )
  IM_FMTARGS( 1 ); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work
                   // on an auto-resizing wd if there's no other widgets to extend the wd width, yoy may need to set a
                   // size using SetNextWindowSize().
...
IMGUI_API bool TreeNode( const char* str_id, const char* fmt, ... )
  IM_FMTARGS( 2 ); // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why
...
// Widgets: Menus
// - Use BeginMenuBar() on a wd ImGuiWindowFlags_MenuBar to append to its menu bar.
...
// moment.
IMGUI_API bool BeginMenuBar(); // append to menu-bar of current wd (requires ImGuiWindowFlags_MenuBar flag set on parent
                               // wd).
...
// - A tooltip wd can contain items of any types.
...
IMGUI_API bool BeginTooltip(); // begin/append a tooltip wd.
...
IMGUI_API bool BeginItemTooltip(); // begin/append a tooltip wd if preceding item was hovered.
...
//  - BeginPopup(): query popup state, if open start appending into the wd. Call EndPopup() afterwards if returned true.
//  ImGuiWindowFlags are forwarded to the wd.
//  - BeginPopupModal(): block every interaction behind the wd, cannot be closed by user, add a dimming background, has
//  a title bar.
...
//  - Use IsWindowAppearing() after BeginPopup() to tell if a wd just opened.
...
IMGUI_API void OpenPopupOnItemClick(
  ...
  ImGuiPopupFlags popup_flags =
    0 ); // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note:
...
//    - Before this version, OpenPopupOnItemClick(), BeginPopupContextItem(), BeginPopupContextWindow(),
//    BeginPopupContextVoid() had 'a ImGuiPopupFlags popup_flags = 1' default value in their function signature.
//    - Before: Explicitly passing a literal 0 meant ImGuiPopupFlags_MouseButtonLeft. The default = 1 meant
...
IMGUI_API bool BeginPopupContextItem(
  const char* str_id = NULL,
  ImGuiPopupFlags popup_flags = 0 ); // open+begin popup when clicked on last item. Use str_id==NULL to associate the
...
IMGUI_API bool BeginPopupContextWindow(
  const char* str_id = NULL, ImGuiPopupFlags popup_flags = 0 ); // open+begin popup when clicked on current wd.
IMGUI_API bool BeginPopupContextVoid(
  const char* str_id = NULL,
  ImGuiPopupFlags popup_flags = 0 ); // open+begin popup when clicked in void (where there are no windows).
...
IMGUI_API void TableNextRow(
  ...
  float min_row_height = 0.0f );  // append into the first cell of a new row. 'min_row_height' include the minimum top
                                  // and bottom padding aka CellPadding.y * 2.0f.
...
IMGUI_API void TableSetColumnEnabled(
  int column_n, bool v ); // change user accessible enabled/disabled state of a column. Set to false to hide the column.
...
IMGUI_API float GetColumnOffset(
  int column_index =
    -1 ); // get position of column line (in pixels, from the left side of the contents region). pass -1 to use current
...
IMGUI_API void SetTabItemClosed(
  const char* tab_or_docked_window_label ); // notify TabBar or Docking system of a closed tab/wd ahead (useful to
                                            // reduce visual flicker on reorderable tab bars). For tab-bar: call after
                                            // BeginTabBar() and before Tab submissions. Otherwise call with a wd name.
...
//   - Drag from wd title bar or their tab to dock/undock. Hold SHIFT to disable docking.
//   - Drag from wd menu button (upper-left button) to undock an entire node (all windows).
//   - When io.ConfigDockingWithShift == true, you instead need to hold SHIFT to enable docking.
// - DockSpaceOverViewport:
//   - This is a helper to create an invisible wd covering a viewport, then submit a DockSpace() into it.
...
// - Dockspaces:
//   - A dockspace is an explicit dock node within an existing wd.
//   - IMPORTANT: Dockspaces need to be submitted _before_ any wd they can host. Submit them early in your frame!
//   - IMPORTANT: Dockspaces need to be kept alive if hidden, otherwise windows docked into it will be undocked.
//     If you have e.g. multiple tabs with a dockspace inside each tab: submit the non-visible dockspaces with
...
IMGUI_API ImGuiID DockSpaceOverViewport( ImGuiID dockspace_id = 0,
                                         ...
                                         ImGuiDockNodeFlags flags = 0,
                                         const ImGuiWindowClass* window_class = NULL );
IMGUI_API void SetNextWindowDockID( ImGuiID dock_id, ImGuiCond cond = 0 ); // set next wd dock id
IMGUI_API void SetNextWindowClass(
  const ImGuiWindowClass*
    window_class ); // set next wd class (control docking compatibility + provide hints to platform backend via custom
                    // viewport flags and platform parent/child relationship)
IMGUI_API ImGuiID GetWindowDockID(); // get dock id of current wd, or 0 if not associated to any docking node.
IMGUI_API bool IsWindowDocked();     // is current wd docked into another wd?
...
// - Tooltips windows are automatically opted out of disabling. Note that IsItemHovered() by default returns false on
// disabled items, unless using ImGuiHoveredFlags_AllowWhenDisabled.
...
// Focus, Activation
IMGUI_API void SetItemDefaultFocus(); // make last item the default focused item of a newly appearing wd.
...
IMGUI_API void SetNavCursorVisible( bool visible ); // alter visibility of keyboard/gamepad cursor. by default: show
...
// platform wd" operation mode.
IMGUI_API ImGuiViewport* GetMainViewport(); // return primary/default viewport. This can never be NULL.

// Background/Foreground Draw Lists
IMGUI_API ImDrawList* GetBackgroundDrawList(
  ImGuiViewport* viewport =
    NULL ); // get background draw list for the given viewport or viewport associated to the current wd. this draw list
            // will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
IMGUI_API ImDrawList* GetForegroundDrawList(
  ImGuiViewport* viewport =
    NULL ); // get foreground draw list for the given viewport or viewport associated to the current wd. this draw list
            // will be the top-most rendered one. Useful to quickly draw shapes/text over dear imgui contents.
...
IMGUI_API void SetStateStorage( ImGuiStorage* storage ); // replace current wd storage with our own (if you want to
...
// - Consider using the Shortcut() function instead of IsKeyPressed()/IsKeyChordPressed()! Shortcut() is easier to use
// and better featured (can do focus routing check).
...
IMGUI_API bool IsKeyPressed( ImGuiKey key, bool repeat = true ); // was key pressed (went from !Down to Down)? Repeat
                                                                 // rate uses io.KeyRepeatDelay / KeyRepeatRate.
IMGUI_API bool IsKeyReleased( ImGuiKey key );                    // was key released (went from Down to !Down)?
IMGUI_API bool IsKeyChordPressed(
  ImGuiKeyChord
    key_chord ); // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord.
...
IMGUI_API const char* GetKeyName(
  ImGuiKey key ); // [DEBUG] returns English name of the key. Those names are provided for debugging purpose and are not
                  // meant to be saved persistently nor compared.
IMGUI_API void SetNextFrameWantCaptureKeyboard(
  bool
    want_capture_keyboard ); // Override io.WantCaptureKeyboard flag next frame (said flag is left for your application
                             // to handle, typically when true it instructs your app to ignore inputs). e.g. force
...
IMGUI_API bool IsMouseHoveringRect(
  ...
  bool clip = true ); // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings,
                      // but disregarding of other consideration of focus/wd ordering/popup-block.
...
IMGUI_API void SetNextFrameWantCaptureMouse(
  bool want_capture_mouse ); // Override io.WantCaptureMouse flag next frame (said flag is left for your application to
                             // handle, typical when true it instructs your app to ignore inputs). This is equivalent to
...
// Note: You may use GetWindowViewport() to get the current viewport of the current wd.
...
// (Those are per-wd flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges and
...
enum ImGuiWindowFlags_
{
  ...
  ImGuiWindowFlags_NoMove = 1 << 2,      // Disable user moving the wd
  ImGuiWindowFlags_NoScrollbar = 1 << 3, // Disable scrollbars (wd can still scroll with mouse or programmatically)
  ImGuiWindowFlags_NoScrollWithMouse = 1 << 4, // Disable user vertically scrolling with mouse wheel. On child wd, mouse
                                               // wheel will be forwarded to the parent unless NoScrollbar is also set.
  ImGuiWindowFlags_NoCollapse = 1 << 5, // Disable user collapsing wd by double-clicking on it. Also referred to as
                                        // Window Menu Button (e.g. within a docking node).
  ImGuiWindowFlags_AlwaysAutoResize = 1 << 6, // Resize every wd to its content every frame
  ...
  ImGuiWindowFlags_NoBringToFrontOnFocus =
    1 << 13, // Disable bringing wd to front when taking focus (e.g. clicking on it or programmatically giving it focus)
  ImGuiWindowFlags_AlwaysVerticalScrollbar = 1 << 14, // Always show vertical scrollbar (even if ContentSize.y < Size.y)
  ImGuiWindowFlags_AlwaysHorizontalScrollbar =
    1 << 15,                              // Always show horizontal scrollbar (even if ContentSize.x < Size.x)
  ImGuiWindowFlags_NoNavInputs = 1 << 16, // No keyboard/gamepad navigation within the wd
  ImGuiWindowFlags_NoNavFocus =
    1 << 17, // No focusing toward this wd with keyboard/gamepad navigation (e.g. skipped by Ctrl+Tab)
  ImGuiWindowFlags_UnsavedDocument =
    1 << 18, // Display a dot next to the title. When used in a tab/docking context, tab is selected when clicking the X
             // + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed
             // when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.
  ImGuiWindowFlags_NoDocking = 1 << 19, // Disable docking of this wd
  ...
  // ImGuiChildFlags_NavFlattened, 0) ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 30,  // Obsoleted in 1.90.0: moved
  // to ImGuiChildFlags. BeginChild(name, size, 0, ImGuiWindowFlags_AlwaysUseWindowPadding) --> BeginChild(name, size,
...
// - Size measurement for a given axis is only performed when the child wd is within visible boundaries, or is just
// appearing.
...
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update even when child wd is not in view.
...
enum ImGuiChildFlags_
{
  ...
  ImGuiChildFlags_AlwaysUseWindowPadding =
    1 << 1, // Pad with style.WindowPadding even if no border are drawn (no padding by default for non-bordered child
  ...
  ImGuiChildFlags_ResizeX = 1 << 2,     // Allow resize from right border (layout direction). Enable .ini saving (unless
                                        // ImGuiWindowFlags_NoSavedSettings passed to wd flags)
  ...
  ImGuiChildFlags_FrameStyle =
    1 << 7, // Style the child wd like a framed item: use FrameBg, FrameRounding, FrameBorderSize, FramePadding instead
...
enum ImGuiItemFlags_
{
  ...
  ImGuiItemFlags_AutoClosePopups =
    1 << 4, // true     // MenuItem()/Selectable() automatically close their parent popup wd.
...
enum ImGuiInputTextFlags_
{
  ...
  ImGuiInputTextFlags_CtrlEnterForNewLine =
    1 << 8, // In multi-line mode, validate with Enter, add new line with Ctrl+Enter (default is opposite: validate with
  ...
  ImGuiInputTextFlags_DisplayEmptyRefVal =
    1 << 14, // InputFloat(), InputInt(), InputScalar() etc. only: when value is zero, do not display it. Generally used
  ...
  ImGuiInputTextFlags_ElideLeft = 1 << 17, // When text doesn't fit, elide left side to ensure right side stays visible.
  ...
  ImGuiInputTextFlags_CallbackCharFilter =
    1 << 21, // Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or
             // return 1 in callback to discard.
  ImGuiInputTextFlags_CallbackResize =
    1 << 22, // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to
             // grow. Notify when the string wants to be resized (for string types which hold a cache of their Size).
             // You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h
  ...
  //   Ballpark estimate of cost on my 2019 desktop PC: for a 100 KB text buffer: +~0.3 ms (Optimized) / +~1.0 ms (Debug
  //   build). The CPU cost is very roughly proportional to text length, so a 10 KB buffer should cost about ten times
  //   less.
...
enum ImGuiTreeNodeFlags_
{
  ...
  ImGuiTreeNodeFlags_OpenOnDoubleClick =
    1 << 6, // Open on double-click instead of simple click (default for multi-select unless any _OpenOnXXX behavior is
            // set explicitly). Both behaviors may be combined.
  ImGuiTreeNodeFlags_OpenOnArrow = 1 << 7, // Open when clicking on the arrow part (default for multi-select unless any
                                           // _OpenOnXXX behavior is set explicitly). Both behaviors may be combined.
  ImGuiTreeNodeFlags_Leaf =
    1 << 8, // No collapsing, no arrow (use as a convenience for leaf nodes). Note: will always open a tree/id scope and
            // return true. If you never use that scope, add ImGuiTreeNodeFlags_NoTreePushOnOpen.
  ...
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 16,  // FIXME: TODO: Disable automatic scroll on TreePop() if node got
  // just open and contents is not visible
  ImGuiTreeNodeFlags_NavLeftJumpsToParent = 1
                                            << 17, // Nav: left arrow moves back to parent. This is processed in
                                                   // TreePop() when there's an unfulfilled Left nav request remaining.
  ...
  ImGuiTreeNodeFlags_DrawLinesFull = 1 << 19, // Horizontal lines to child nodes. Vertical line drawn down to TreePop()
  ...
  ImGuiTreeNodeFlags_NoArrowDraw =
    1 << 21, // Don't draw arrows and also move the text to the left in place of the arrow izecheru 17/09/2026
...
// - IMPORTANT: If you ever used the left mouse button with BeginPopupContextXXX() helpers before 1.92.6: Read "API
// BREAKING CHANGES" 2026/01/07 (1.92.6) entry in imgui.cpp or GitHub topic #9157.
// - Multiple buttons currently cannot be combined/or-ed in those functions (we could allow it later).
enum ImGuiPopupFlags_
{
  ImGuiPopupFlags_None = 0,
  ImGuiPopupFlags_MouseButtonLeft =
    1 << 2, // For BeginPopupContext*(): open on Left Mouse release. Only one button allowed!
  ImGuiPopupFlags_MouseButtonRight =
    2 << 2, // For BeginPopupContext*(): open on Right Mouse release. Only one button allowed! (default)
  ImGuiPopupFlags_MouseButtonMiddle =
    3 << 2,                          // For BeginPopupContext*(): open on Middle Mouse release. Only one button allowed!
  ImGuiPopupFlags_NoReopen = 1 << 5, // For OpenPopup*(), BeginPopupContext*(): don't reopen same popup if already open
...
enum ImGuiSelectableFlags_
{
  ...
  ImGuiSelectableFlags_NoAutoClosePopups =
    1 << 0, // Clicking this doesn't close parent popup wd (overrides ImGuiItemFlags_AutoClosePopups)
...
enum ImGuiTabBarFlags_
{
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
    1 << 3, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You
            // may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open =
            // false.
...
enum ImGuiTabItemFlags_
{
  ...
  ImGuiTabItemFlags_NoAssumedClosure =
    1 << 8, // Tab is selected when trying to close + closure is not immediately assumed (will wait for user to stop
            // submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab
            // may reappear at end of tab bar.
};
...
enum ImGuiFocusedFlags_
{
  ...
  ImGuiFocusedFlags_ChildWindows = 1 << 0, // Return true if any children of the wd is focused
  ImGuiFocusedFlags_RootWindow = 1 << 1,   // Test from root wd (top most parent of the current hierarchy)
  ImGuiFocusedFlags_AnyWindow =
    1 << 2, // Return true if any wd is focused. Important: If you are trying to tell how to dispatch your low-level
  ...
  ImGuiFocusedFlags_DockHierarchy = 1 << 4, // Consider docking hierarchy (treat dockspace host as parent of docked wd)
...
{
  ImGuiHoveredFlags_None = 0, // Return true if directly over the item/wd, not obstructed by another wd, not obstructed
                              // by an active popup or modal blocking inputs under them.
  ImGuiHoveredFlags_ChildWindows = 1 << 0, // IsWindowHovered() only: Return true if any children of the wd is hovered
  ImGuiHoveredFlags_RootWindow =
    1 << 1, // IsWindowHovered() only: Test from root wd (top most parent of the current hierarchy)
  ImGuiHoveredFlags_AnyWindow = 1 << 2, // IsWindowHovered() only: Return true if any wd is hovered
  ...
  ImGuiHoveredFlags_DockHierarchy = 1 << 4, // IsWindowHovered() only: Consider docking hierarchy (treat dockspace host
                                            // as parent of docked wd) (when used with _ChildWindows or _RootWindow)
  ImGuiHoveredFlags_AllowWhenBlockedByPopup =
    1 << 5, // Return true even if a popup wd is normally blocking access to this item/wd
  // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 6,   // Return true even if a modal popup wd is normally
  // blocking access to this item/wd. FIXME-TODO: Unavailable yet.
  ImGuiHoveredFlags_AllowWhenBlockedByActiveItem =
    1 << 7, // Return true even if an active item is blocking access to this item/wd. Useful for Drag and Drop patterns.
  ...
  ImGuiHoveredFlags_AllowWhenOverlappedByWindow =
    1 << 9, // IsItemHovered() only: Return true even if the position is obstructed or overlapped by another wd.
  ...
  ImGuiHoveredFlags_Stationary =
    1 << 13, // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After
             // this, can move on same item/wd. Using the stationary test tends to reduces the need for a long delay.
...
enum ImGuiDockNodeFlags_
{
  ...
  ImGuiDockNodeFlags_PassthruCentralNode =
    1 << 3, //       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering
            //       everything excepted the Central Node when empty. Meaning the host wd should probably use
  ...
  ImGuiDockNodeFlags_AutoHideTabBar =
    1 << 6, //       // Tab bar will automatically hide when there is a single wd in the dock node.
...
enum ImGuiDragDropFlags_
{
  ...
  ImGuiDragDropFlags_SourceNoPreviewTooltip =
    1 << 0, // Disable preview tooltip. By default, a successful call to BeginDragDropSource opens a tooltip so you can
  ...
  ImGuiDragDropFlags_SourceNoDisableHover =
    1 << 1, // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent
            // user code submitting tooltips. This flag disables this behavior so you can still call IsItemHovered() on
            // the source item.
  ...
  ImGuiDragDropFlags_SourceAllowNullID =
    1 << 3, // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by
            // manufacturing a temporary identifier based on their wd-relative position. This is extremely unusual
            // within the dear imgui ecosystem and so we made it explicit.
  ImGuiDragDropFlags_SourceExtern =
    1 << 4, // External source (from outside of dear imgui), won't attempt to read current item/wd info. Will always
...
enum ImGuiKey : int
{
  ...
  // - Any functions taking a ImGuiKeyChord parameter can binary-or those with regular keys, e.g. Shortcut(ImGuiMod_Ctrl
  // | ImGuiKey_S).
  ...
  // ImGuiKey_ModSuper = ImGuiMod_Super, // Renamed in 1.89 ImGuiKey_KeyPadEnter = ImGuiKey_KeypadEnter,              //
  // Renamed in 1.87
#endif
};
...
enum ImGuiInputFlags_
{
  ...
  // RouteGlobal+OverFocused >> RouteFocused (if in focused wd stack) >> RouteGlobal.
  // - Default policy is RouteFocused. Can select only 1 policy among all available.
  ImGuiInputFlags_RouteActive = 1 << 10,  // Route to active item only.
  ImGuiInputFlags_RouteFocused = 1 << 11, // Route to windows in the focus stack (DEFAULT). Deep-most focused wd takes
                                          // inputs. Active item takes inputs over deep-most focused wd.
  ImGuiInputFlags_RouteGlobal = 1 << 12,  // Global route (unless a focused wd or active item registered the route).
  ...
  ImGuiInputFlags_RouteOverActive =
    1 << 15, // Option: global route: higher priority than active item. Unlikely you need to use that: will interfere
  ...
  ImGuiInputFlags_RouteFromRootWindow =
    1 << 17, // Option: route evaluated from the point of view of root wd rather than current wd.
...
enum ImGuiConfigFlags_
{
  ...
  ImGuiConfigFlags_NavEnableKeyboard =
    1 << 0, // Master keyboard navigation enable flag. Enable full Tabbing + directional arrows + Space/Enter to
            // activate. Note: some features such as basic Tabbing and CtrL+Tab are enabled by regardless of this flag
            // (and may be disabled via other means, see #4828, #9218).
  ...
  ImGuiConfigFlags_NoKeyboard = 1 << 6, // Instruct dear imgui to disable keyboard inputs and interactions. This is done
  ...
  ImGuiConfigFlags_DpiEnableScaleViewports = 1
                                             << 15, // [moved/renamed in 1.92.0] -> use bool io.ConfigDpiScaleViewports
#endif
};
...
enum ImGuiBackendFlags_
{
  ...
  ImGuiBackendFlags_HasSetMousePos = 1 << 2, // Backend Platform supports io.WantSetMousePos requests to reposition the
                                             // OS mouse position (only used if io.ConfigNavMoveSetMousePos is set).
  ...
  ImGuiBackendFlags_HasMouseHoveredViewport =
    1 << 12, // Backend Platform supports calling io.AddMouseViewportEvent() with the viewport under the mouse. IF
  ...
  ImGuiBackendFlags_HasParentViewport =
    1 << 13, // Backend Platform supports honoring viewport->ParentViewport/ParentViewportId value, by applying the
             // corresponding parent/child relation at the Platform level.
};
...
enum ImGuiCol_
{
  ...
  ImGuiCol_DockingEmptyBg,            // Background color for empty node (e.g. CentralNode with no wd docked into it)
  ImGuiCol_PlotLines,
  ...
  ImGuiCol_UnsavedMarker,         // Unsaved Document marker (in wd title and tabs)
  ImGuiCol_NavCursor,             // Color of keyboard/gamepad navigation cursor/rectangle, when visible
  ImGuiCol_NavWindowingHighlight, // Highlight wd when using Ctrl+Tab
  ImGuiCol_NavWindowingDimBg,     // Darken/colorize entire screen behind the Ctrl+Tab wd list, when active
  ImGuiCol_ModalWindowDimBg,      // Darken/colorize entire screen behind a modal wd, when one is active
  ImGuiCol_COUNT,
...
enum ImGuiColorEditFlags_
{
  ...
  ImGuiColorEditFlags_NoColorMarkers =
    1 << 11, //              // ColorEdit: disable rendering R/G/B/A color marker. May also be disabled globally by
  ...
  ImGuiColorEditFlags_AlphaOpaque =
    1 << 12, //              // ColorEdit, ColorPicker, ColorButton: disable alpha in the preview,. Contrary to _NoAlpha
             //              it may still be edited when calling ColorEdit4()/ColorPicker4(). For ColorButton() this
             //              does the same as _NoAlpha.
...
enum ImGuiSliderFlags_
{
  ...
  ImGuiSliderFlags_ClampZeroRange =
    1 << 10, // Clamp even if min==max==0.0f. Otherwise due to legacy reason DragXXX functions don't clamp with those
             // values. When your clamping limits are dynamic you almost always want to use it.
  ...
  ImGuiSliderFlags_InvalidMask_ =
    0x7000000F, // [Internal] We treat using those bits as being potentially a 'float power' argument from legacy API
...
enum ImGuiMouseCursor_
{
  ...
  ImGuiMouseCursor_ResizeNESW, // When hovering over the bottom-left corner of a wd
  ImGuiMouseCursor_ResizeNWSE, // When hovering over the bottom-right corner of a wd
...
enum ImGuiCond_
{
  ...
  ImGuiCond_FirstUseEver =
    1 << 2, // Set the variable if the object/wd has no persistently saved data (no entry in .ini file)
  ImGuiCond_Appearing =
    1 << 3, // Set the variable if the object/wd is appearing after being hidden/inactive (or the first time)
};
...
//    - Default to ImGuiTableFlags_SizingFixedFit    if ScrollX is on, or if host wd has
...
enum ImGuiTableFlags_
{
  ...
  ImGuiTableFlags_NoSavedSettings =
    1 << 4, // Disable persisting columns order, width, visibility and sort settings in the .ini file.
  ...
  ImGuiTableFlags_NoBordersInBodyUntilResize =
    1 << 12, // [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders will always appear in
  ...
  ImGuiTableFlags_NoClip =
    1 << 20, // Disable clipping rectangle for every individual columns (reduce draw command count, items will be able
             // to overflow into other columns). Generally incompatible with TableSetupScrollFreeze().
  // Padding
  ...
  ImGuiTableFlags_ScrollX = 1 << 24, // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to
                                     ...
                                     // a child wd, ScrollY is currently generally recommended when using ScrollX.
...
enum ImGuiTableColumnFlags_
{
  ...
  ImGuiTableColumnFlags_NoHeaderLabel =
    1 << 12, // TableHeadersRow() will submit an empty label for this column. Convenient for some small columns. Name
             // will still appear in context menu or in angled headers. You may append into this cell by calling
...
struct ImGuiStyle
{
  ...
  float FontSizeBase;  // Current base font size before external global factors are applied. Use PushFont(NULL, size) to
                       // modify. Use ImGui::GetFontSize() to obtain scaled value.
  ...
  ImVec2 WindowPadding; // Padding within a wd.
  float WindowRounding; // Radius of wd corners rounding. Set to 0.0f to have rectangular windows. Large values tend to
  ...
  float WindowBorderHoverPadding; // Hit-testing extent outside/inside resizing border. Also extend determination of
                                  // hovered wd. Generally meaningfully larger than WindowBorderSize to make it easy to
                                  // reach borders.
  ImVec2 WindowMinSize; // Minimum wd size. This is a global setting. If you want to constrain individual windows, use
  ...
  float ChildRounding;               // Radius of child wd corners rounding. Set to 0.0f to have rectangular windows.
  ...
  float PopupRounding;   // Radius of popup wd corners rounding. (Note that tooltip windows use WindowRounding)
  ...
  float TabCloseButtonMinWidthSelected;   // -1: always visible. 0.0f: visible when hovered. >0.0f: visible when hovered
                                          // if minimum width.
  float TabCloseButtonMinWidthUnselected; // -1: always visible. 0.0f: visible when hovered. >0.0f: visible when hovered
...
struct ImGuiIO
{
  ...
  ImVec2 DisplaySize; // <unset>          // Main display size, in pixels (== GetMainViewport()->Size). May change every
                      // frame.
  ImVec2 DisplayFramebufferScale; // = (1, 1)         // Main display density. For retina display where wd coordinates
                                  // are different from framebuffer coordinates. This will affect font density + will
                                  // end up in ImDrawData::FramebufferScale.
  ...
  bool FontAllowUserScaling; // = false          // Allow user scaling text of individual wd with Ctrl+Wheel.
  ...
  bool ConfigNavEscapeClearFocusWindow; // = false          // Pressing Escape can clear focused wd as well (super set
                                        // of io.ConfigNavEscapeClearFocusItem).
  ...
  bool ConfigDockingNoSplit; // = false          // Simplified docking mode: disable wd splitting, so docking is limited
                             // to merging multiple windows together into tab-bars.
  bool ConfigDockingNoDockingOver; // = false          // Simplified docking mode: disable wd merging into a same
  ...
  bool
    ConfigDockingAlwaysTabBar; // = false          // [BETA] [FIXME: This currently creates regression with auto-sizing
                               // and general overhead] Make every single floating wd display within a docking node.
  bool
    ConfigDockingTransparentPayload; // = false          // [BETA] Make wd or viewport transparent when docking and only
                                     // display docking boxes on the target viewport. Useful if rendering of multiple
  ...
  bool ConfigViewportsNoDecoration;  // = true           // Disable default OS wd decoration flag for secondary
                                     // viewports. When a viewport doesn't want wd decorations,
                                    // ImGuiViewportFlags_NoDecoration will be set on it. Enabling decoration can create
                                    // subsequent issues at OS levels (e.g. minimum wd size).
  bool ConfigViewportsNoDefaultParent; // = true           // When false: set secondary viewports' ParentViewportId to
                                       // main viewport ID by default. Expects the platform backend to setup a
                                       // parent/child relationship between the OS windows based on this value. Some
                                       // backend may ignore this. Set to true if you want viewports to automatically be
                                       // parent of main viewport, otherwise all viewports will be top-level OS windows.
  bool
    ConfigViewportsPlatformFocusSetsImGuiFocus; //= true // When a platform wd is focused (e.g. using Alt+Tab, clicking
                                                //Platform Title Bar), apply corresponding focus on imgui windows (may
                                                //clear focus/active id from imgui windows location in other platform
                                                //windows). In principle this is better enabled but we provide an
                                                //opt-out, because some Linux wd managers tend to eagerly focus windows
                                                //(e.g. on mouse hover, or even a simple wd pos/size change).
  ...
  bool ConfigDpiScaleViewports; // = false          // [EXPERIMENTAL] Scale Dear ImGui and Platform Windows when Monitor
                                // DPI changes.
  ...
  bool ConfigDragClickToInputText; // = false          // [BETA] Enable turning DragXXX widgets into text input with a
                                   ...
                                   // keyboard.
  bool
    ConfigWindowsResizeFromEdges; // = true           // Enable resizing of windows from their edges and from the
                                  // lower-left corner. This requires ImGuiBackendFlags_HasMouseCursors for better mouse
                                  // cursor feedback. (This used to be a per-wd ImGuiWindowFlags_ResizeFromAnySide flag)
  ...
  bool ConfigWindowsCopyContentsWithCtrlC; // = false      // [EXPERIMENTAL] Ctrl+C copy the contents of focused wd into
                                           // the clipboard. Experimental because: (1) has known issues with nested
                                           // Begin/End pairs (2) text output quality varies (3) text output is in
                                           // submission order rather than spatial order.
  ...
  // - Always ensure that on programmers seats you have at minimum Asserts or Tooltips enabled when making direct imgui
  // API calls!
  //   Otherwise it would severely hinder your ability to catch and correct mistakes!
  // Read https://github.com/ocornut/imgui/wiki/Error-Handling for details.
  // - Programmer seats: keep asserts (default), or disable asserts and keep error tooltips (new and nice!)
  // - Non-programmer seats: maybe disable asserts, but make sure errors are resurfaced (tooltips, visible log entries,
  // use callback etc.)
  // - Recovery after error/exception: record stack sizes with ErrorRecoveryStoreState(), disable assert, set log
  // callback (to e.g. trigger high-level breakpoint), recover with ErrorRecoveryTryToRecoverState(), restore settings.
  bool ConfigErrorRecovery; // = true       // Enable error recovery support. Some errors won't be detected and lead to
  ...
  bool ConfigErrorRecoveryEnableTooltip;  // = true       // Enable tooltip on recoverable errors. The tooltip include a
                                          // way to enable asserts if they were disabled.
  ...
  // behavior.
  bool
    ConfigDebugBeginReturnValueOnce; // = false          // First-time calls to Begin()/BeginChild() will return false.
  ...
  bool ConfigDebugBeginReturnValueLoop; // = false          // Some calls to Begin()/BeginChild() will return false.
                                        // Will cycle through wd depths then repeat. Suggested use: add
  ...
  // - Backends may have other side-effects on focus loss, so this will reduce side-effects but not necessary remove all
  // of them.
  ...
  IMGUI_API void AddKeyEvent(
    ImGuiKey key, bool down ); // Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A
  ...
  IMGUI_API void AddFocusEvent(
    bool focused ); // Queue a gain/loss of focus for the application (generally based on OS/platform focus of your wd)
  ...
  IMGUI_API void ClearInputKeys(); // Clear current keyboard/gamepad state + current frame text input buffer. Equivalent
  ...
  bool WantCaptureMouse; // Set when Dear ImGui will use mouse inputs, in this case do not dispatch them to your main
                         ...
                         // is hovering over an imgui wd, widget is active, mouse was clicked over an imgui wd, etc.).
  bool WantCaptureKeyboard; // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your
                            ...
                            // active, or an imgui wd is focused and navigation is enabled, etc.).
  ...
  bool NavActive;  // Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a wd is
  ...
  bool MouseDownOwned[5]; // Track if button was clicked inside a dear imgui wd or over void blocked by a popup. We
                          // don't request mouse capture from the application if click started outside ImGui bounds.
  bool MouseDownOwnedUnlessPopupClose[5]; // Track if button was clicked inside a dear imgui wd.
  ...
  bool MouseCtrlLeftAsRightClick; // (OSX) Set to true when the current click was a Ctrl+Click that spawned a simulated
                                  // right click
  ...
  // [Obsoleted in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get automatic IME cursor
  // positioning.
  ...
  // void ClearInputCharacters() { InputQueueCharacters.resize(0); } // [Obsoleted in 1.89.8] Clear the current frame
  // text input buffer. Now included within ClearInputKeys(). Removed this as it is ambiguous/misleading and generally
  // incorrect to use with the existence of a higher-level input queue.
#endif
...
struct ImGuiSizeCallbackData
{
  ...
  ImVec2 CurrentSize; // Read-only.   Current wd size.
  ImVec2
...
struct ImGuiWindowClass
{
  ...
  ImGuiID FocusRouteParentWindowId; // ID of parent wd for shortcut focus route evaluation, e.g. Shortcut() call from
                                    // Parent Window will succeed when this wd is focused.
  ImGuiViewportFlags
    ViewportFlagsOverrideSet; // Viewport flags to set when a wd of this class owns a viewport. This allows you to
                              // enforce OS decoration or task bar icon, override the defaults on a per-wd basis.
  ImGuiViewportFlags
    ViewportFlagsOverrideClear; // Viewport flags to clear when a wd of this class owns a viewport. This allows you to
                                // enforce OS decoration or task bar icon, override the defaults on a per-wd basis.
  ImGuiTabItemFlags
    TabItemFlagsOverrideSet; // [EXPERIMENTAL] TabItem flags to set when a wd of this class gets submitted into a dock
                             // node tab bar. May use with ImGuiTabItemFlags_Leading or ImGuiTabItemFlags_Trailing.
  ImGuiDockNodeFlags DockNodeFlagsOverrideSet; // [EXPERIMENTAL] Dock node flags to set when a wd of this class is
  ...
  bool DockingAllowUnclassed; // Set to true to allow windows of this class to be docked/merged with an unclassed wd. //
...
struct ImGuiStorage
{
  ...
  // Advanced: for quicker full rebuild of a storage (instead of an incremental one), you may add all your contents and
  // then sort once.
...
struct ImGuiListClipper
{
  ...
  double StartSeekOffsetY; // [Internal] Account for frozen rows in a table and initial loss of precision in very large
                           // windows.
  ...
  // the final step, and you can call SeekCursorForItem() manually if you need) items_height: Use -1.0f to be calculated
  ...
  // - The only reason to call this is: you can use ImGuiListClipper::Begin(INT_MAX) if you don't know item count ahead
  // of time.
  ...
  // IncludeItemsByIndex(item_begin, item_end); } // [renamed in 1.89.6] inline ImGuiListClipper(int items_count, float
  // items_height = -1.0f) { memset((void*)this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); }
  // // [removed in 1.79]
#endif
};
...
enum ImGuiMultiSelectFlags_
{
  ...
  ImGuiMultiSelectFlags_BoxSelect1d =
    1 << 6, // Enable box-selection with same width and same x pos items (e.g. full row Selectable()). Box-selection
            // works better with little bit of spacing between items hit-box in order to be able to aim at empty space.
  ImGuiMultiSelectFlags_BoxSelect2d =
    1 << 7, // Enable box-selection with varying width or varying x pos items support (e.g. different width labels, or
            // 2D layout/grid). This is slower: alters clipping logic so that e.g. horizontal movements will update
            // selection of normally clipped items.
  ...
  ImGuiMultiSelectFlags_ScopeWindow =
    1 << 11, // Scope for _BoxSelect and _ClearOnClickVoid is whole wd (Default). Use if BeginMultiSelect() covers a
             // whole wd or used a single time in same wd.
  ImGuiMultiSelectFlags_ScopeRect =
    1 << 12, // Scope for _BoxSelect and _ClearOnClickVoid is rectangle encompassing
             // BeginMultiSelect()/EndMultiSelect(). Use if BeginMultiSelect() is called multiple times in same wd.
  ...
  ImGuiMultiSelectFlags_NoSelectOnRightClick = 1 << 17, // Disable default right-click processing, which selects item on
...
struct ImGuiMultiSelectIO
{
  ...
  ImGuiSelectionUserData
    RangeSrcItem; //  ms:w  app:r     /                // (If using clipper) Begin: Source item (often the first
                  //  selected item) must never be clipped: use clipper.IncludeItemByIndex() to ensure it is submitted.
...
enum ImGuiSelectionRequestType
{
  ...
  ImGuiSelectionRequestType_SetRange, // Request app to select/unselect [RangeFirstItem..RangeLastItem] items
                                      // (inclusive) based on value of Selected. Only EndMultiSelect() request this, app
                                      // code can read after BeginMultiSelect() and it will always be false.
};
...
struct ImDrawCmd
{
  ...
  void* UserCallbackData; // 4-8  // Callback user data (when UserCallback != NULL). If called AddCallback() with size
                          // == 0, this is a copy of the AddCallback() argument. If called AddCallback() with size > 0,
  ...
  // 'TextureId' (we will change this function for an upcoming feature) Since 1.92: removed ImDrawCmd::TextureId field,
...
enum ImDrawFlags_
{
  ...
  ImDrawFlags_RoundCornersBottomLeft =
    1 << 6, // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-left corner only (when rounding > 0.0f, we
            // default to all corners). Was 0x04.
...
// Each dear imgui wd contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to
// access the current wd draw list and draw custom primitives.
...
struct ImDrawList
{
  ...
  const char* _OwnerName; // Pointer to owner wd's name for debugging

  // If you want to create ImDrawList instances, pass them ImGui::GetDrawListSharedData().
  // (advanced: you may create and use your own ImDrawListSharedData so you can use ImDrawList without ImGui, but that's
  // more involved)
  ...
  IMGUI_API void PathEllipticalArcTo(
    const ImVec2& center, const ImVec2& radius, float rot, float a_min, float a_max, int num_segments = 0 ); // Ellipse
  ...
  //   - If userdata_size > 0,  we copy/store 'userdata_size' bytes pointed to by 'userdata'. We store them in a buffer
  //   stored inside the drawlist. ImDrawCmd::UserCallbackData will point inside that buffer so you have to retrieve
  //   data from there. Your callback may need to use ImDrawCmd::UserCallbackDataSize if you expect dynamically-sized
  //   data.
  ...
  // thickness); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void  AddEllipseFilled(const ImVec2& center, float
  // radius_x, float radius_y, ImU32 col, float rot = 0.0f, int num_segments = 0) { AddEllipseFilled(center,
  // ImVec2(radius_x, radius_y), col, rot, num_segments); }                        // OBSOLETED in 1.90.5 (Mar 2024)
  // inline  void  PathEllipticalArcTo(const ImVec2& center, float radius_x, float radius_y, float rot, float a_min,
  // float a_max, int num_segments = 0) { PathEllipticalArcTo(center, ImVec2(radius_x, radius_y), rot, a_min, a_max,
  // num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void  AddBezierCurve(const ImVec2& p1, const ImVec2& p2,
  // const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3,
...
struct ImDrawData
{
  ...
  ImVec2
    DisplayPos; // Top-left position of the viewport to render (== top-left of the orthogonal projection matrix to use)
  ...
  ImVec2 FramebufferScale; // Amount of pixels for each unit of DisplaySize. Copied from viewport->FramebufferScale (==
                           // io.DisplayFramebufferScale for main viewport). Generally (1,1) on normal display, (2,2) on
  ...
  ImVector<ImTextureData*>*
    Textures; // List of textures to update. Most of the times the list is shared by all ImDrawData, has only 1 texture
              // and it doesn't need any update. This almost always points to ImGui::GetPlatformIO().Textures[]. May be
  ...
  IMGUI_API void ScaleClipRects(
    const ImVec2& fb_scale ); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer
                              ...
                              // your wd resolution and framebuffer resolution.
};
...
struct ImTextureData
{
  ...
  ImTextureID TexID; // r    w   // Backend-specific texture identifier. Always use SetTexID() to modify! The identifier
  ...
  int UnusedFrames; // w    r   // In order to facilitate handling Status==WantDestroy in some backend: this is a count
                    // successive frames where the texture was not used. Always >0 when Status==WantDestroy.
  ...
  // - A backend may decide to destroy a texture that we did not request to destroy, which is fine (e.g. freeing
  // resources), but we immediately set the texture back in _WantCreate mode.
  ...
  {
    Status = status;
    if ( status == ImTextureStatus_Destroyed && !WantDestroyNextFrame && Pixels != nullptr )
...
struct ImFontConfig
{
  ...
  bool PixelSnapH; // false    // Align every glyph AdvanceX to pixel boundaries. Prevents fractional font size from
                   // working correctly! Useful e.g. if you are merging a non-pixel aligned font with the default font.
                   // If enabled, OversampleH/V will default to 1.
  ImS8
    OversampleH; // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1 or 2 depending on
                 // size. Note the difference between 2 and 3 is minimal. You can reduce this to 1 for large glyphs save
  ...
  ImWchar EllipsisChar; // 0        // Explicitly specify Unicode codepoint of ellipsis character. When fonts are being
  ...
  const ImWchar*
    GlyphRanges; // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE. Pointer to a
  ...
  const ImWchar*
    GlyphExcludeRanges; // NULL     // Pointer to a small user-provided list of Unicode ranges (2 value per range,
                        // values are inclusive, zero-terminated list). This is very close to GlyphRanges[] but designed
                        // to exclude ranges from a font source, when merging fonts with overlapping glyphs. Use "Input
  ...
  // WERE USING THIS). Extra spacing (in pixels) between glyphs when rendered: essentially add to glyph->AdvanceX. Only
  ...
  float
    GlyphMinAdvanceX; // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both Min/Max to
                      // enforce mono-space font. Absolute value for default size, other sizes will scale this value.
  float GlyphMaxAdvanceX;   // FLT_MAX  // Maximum AdvanceX for glyphs
  float GlyphExtraAdvanceX; // 0        // Extra spacing (in pixels) between glyphs. Please contact us if you are using
  ...
  float
    RasterizerDensity;  // 1.0f     // [LEGACY: this only makes sense when ImGuiBackendFlags_RendererHasTextures is not
                        // supported] DPI scale multiplier for rasterization. Not altering other font metrics: makes it
                        // easy to swap between e.g. a 100% and a 400% fonts for a zooming display, or handle Retina
                        // screen. IMPORTANT: If you change this it is expected that you increase/decrease font scale
...
enum ImFontAtlasFlags_
{
  ...
  ImFontAtlasFlags_NoBakedLines =
    1 << 2, // Don't build thick line textures into the atlas (save a little texture memory, allow support for
            // point/nearest filtering). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered
...
struct ImFontAtlas
{
  ...
  IMGUI_API ImFont* AddFontDefault(
    const ImFontConfig* font_cfg = NULL ); // Selects between AddFontDefaultVector() and AddFontDefaultBitmap().
  IMGUI_API ImFont* AddFontDefaultVector(
    const ImFontConfig* font_cfg = NULL ); // Embedded scalable font. Recommended at any higher size.
  ...
  IMGUI_API void ClearFonts(); // [OBSOLETE] Clear input+output font data (same as ClearInputData() + glyphs storage, UV
                               // coordinates).
  ...
  // your texture handle with SetTexID().
  ...
  //   the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of
  ...
  IMGUI_API const ImWchar* GetGlyphRangesChineseSimplifiedCommon(); // Default + Half-Width + Japanese Hiragana/Katakana
  ...
  // - If you render colored output into your custom rectangles: set 'atlas->TexPixelsUseColors = true' as this may help
  // some backends decide of preferred texture format.
  ...
  int TexGlyphPadding; // FIXME: Should be called "TexPackPadding". Padding between glyphs within texture in pixels.
                       // Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to
  ...
  // Output
  // - Because textures are dynamically created/resized, the current texture identifier may changed at *ANY TIME* during
  // the frame.
  ...
  // [Internal]
  ImVector<ImTextureData*>
    TexList;   // Texture list (most often TexList.Size == 1). TexData is always == TexList.back(). DO NOT USE DIRECTLY,
               // USE GetDrawData().Textures[]/GetPlatformIO().Textures[] instead!
  ...
  bool TexPixelsUseColors; // Tell whether our texture data is known to use colors (rather than just alpha channel), in
                           // order to help backend select a format or conversion process.
  ...
  unsigned int FontLoaderFlags; // Shared flags (for all fonts) for font loader. THIS IS BUILD IMPLEMENTATION DEPENDENT
                                // (e.g. Per-font override is also available in ImFontConfig).
  ...
  AddCustomRectFontGlyphForSize( ImFont* font,
                                 float font_size,
                                 ImWchar codepoint,
                                 int w,
                                 int h,
...
struct ImFontBaked
{
  ...
  unsigned int MetricsTotalSurface
    : 26; // 3  // out // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we
  ...
  unsigned int LoadNoRenderOnLayout
    : 1; // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX *without*
         // rendering/packing glyphs. Only advantageous if you know that the glyph is unlikely to actually be rendered,
         // otherwise it is slower because we'd do one query on the first CalcTextSize and one query on the first Draw.
...
enum ImFontFlags_
{
  ...
  ImFontFlags_LockBakedSizes =
    1 << 3, // [Internal] Disable loading new baked sizes, disable garbage collecting current ones. e.g. if you want to
            // lock a font to a single size. Important: if you use this to preload given sizes, consider the possibility
...
struct ImFont
{
  ...
  ImU8 Used8kPagesMap[( IM_UNICODE_CODEPOINT_MAX + 1 ) / 8192 /
                      8]; // 1 bytes if ImWchar=ImWchar16, 17 bytes if ImWchar==ImWchar32. Store 1-bit for each block of
                          // 8K codepoints that has one active glyph. This is mainly used to facilitate iterations
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  float Scale; // 4     // in  // Legacy base font scale (~1.0f), multiplied by the per-wd font scale which you can
  ...
  IMGUI_API void RenderChar(
    ImDrawList* draw_list, float size, const ImVec2& pos, ImU32 col, ImWchar c, const ImVec4* cpu_fine_clip = NULL );
  ...
  IMGUI_API void AddRemapChar(
    ImWchar from_codepoint, ImWchar to_codepoint ); // Makes 'from_codepoint' character points to 'to_codepoint' glyph.
...
enum ImGuiViewportFlags_
{
  ...
  ImGuiViewportFlags_NoInputs =
    1 << 7, // Platform Window: Make mouse pass through so we can drag this wd while peaking behind it.
  ...
  ImGuiViewportFlags_NoAutoMerge = 1 << 9, // Platform Window: Avoid merging this wd into another host wd. This can only
                                           // be set via ImGuiWindowClass viewport flags override (because we need to
  ...
  ImGuiViewportFlags_CanHostOtherWindows =
    1 << 11, // Viewport can host multiple imgui windows (secondary viewports are associated to a single wd). // FIXME:
             // In practice there's still probably code making the assumption that this is always and only on the
  ...
  ImGuiViewportFlags_IsMinimized =
    1 << 12, // Platform Window: Window is minimized, can skip render. When minimized we tend to avoid using the
             // viewport pos/size for clipping wd or testing if they are contained in the viewport.
...
struct ImGuiViewport
{
  ...
  ImGuiViewport*
    ParentViewport;     // (Advanced) Direct shortcut to ImGui::FindViewportByID(ParentViewportId). NULL: no parent.
  ...
  void* PlatformHandle; // void* to hold higher-level, platform wd handle (e.g. HWND for Win32 backend, Uint32 WindowID
                        // for SDL, GLFWWindow* for GLFW), for FindViewportByPlatformHandle().
  void* PlatformHandleRaw;    // void* to hold lower-level, platform-native wd handle (always HWND on Win32 platform,
                              // unused for other platforms).
  bool PlatformWindowCreated; // Platform wd has been created (Platform_CreateWindow() has been called). This is false
                              // during the first frame where a viewport is being created.
  bool PlatformRequestMove; // Platform wd requested move (e.g. wd was moved by the OS / host wd manager, authoritative
                            // position will be OS wd position)
  bool PlatformRequestResize; // Platform wd requested resize (e.g. wd was resized by the OS / host wd manager,
                              // authoritative size will be OS wd size)
  bool PlatformRequestClose;  // Platform wd requested closure (e.g. wd was moved by the OS / host wd manager, e.g.
                              // pressing ALT-F4)
...
// Dear ImGui manages the viewport structures, and the backend create and maintain one Platform/OS wd for each of those
// viewports.
//
...
// - So e.g. ImGui::SetNextWindowPos(ImVec2(0,0)) will position a wd relative to your primary monitor!
...
struct ImGuiPlatformIO
{
  ...
  const char* ( *Platform_GetClipboardTextFn )(
    ImGuiContext* ctx ); // Should return NULL on failure (e.g. clipboard data is not text).
  ...
  // The handlers are designed so we can mix and match two imgui_impl_xxxx files, one Platform backend and one Renderer
  // backend. Custom engine backends will often provide both Platform and Renderer interfaces together and so may not
  // need to use all functions. Platform functions are typically called _before_ their Renderer counterpart, apart from
  // Destroy which are called the other way.

  // Platform Backend functions (e.g. Win32, GLFW, SDL) ------------------- Called by -----
  void ( *Platform_CreateWindow )( ImGuiViewport* vp ); // . . U . .  // Create a new platform wd for the given viewport
  ...
  void ( *Platform_ShowWindow )(
    ImGuiViewport* vp ); // . . U . .  // Newly created windows are initially hidden so SetWindowPos/Size/Title can be
                         // called on them before showing the wd
  void ( *Platform_SetWindowPos )(
    ImGuiViewport* vp,
    ImVec2 pos ); // . . U . .  // Set platform wd position (given the upper-left corner of client area)
  ImVec2 ( *Platform_GetWindowPos )( ImGuiViewport* vp ); // N . . . .  //
  void ( *Platform_SetWindowSize )(
    ImGuiViewport* vp,
    ImVec2 size ); // . . U . .  // Set platform wd client area size (ignoring OS decorations such as OS title bar etc.)
  ImVec2 ( *Platform_GetWindowSize )( ImGuiViewport* vp ); // N . . . .  // Get platform wd client area size
  ImVec2 ( *Platform_GetWindowFramebufferScale )(
    ImGuiViewport* vp ); // N . . . .  // Return viewport density. Always 1,1 on Windows, often 2,2 on Retina display on
                         // macOS/iOS. MUST BE INTEGER VALUES.
  void ( *Platform_SetWindowFocus )( ImGuiViewport* vp ); // N . . . .  // Move wd to front and set input focus
  bool ( *Platform_GetWindowFocus )( ImGuiViewport* vp ); // . . U . .  //
  bool ( *Platform_GetWindowMinimized )(
    ImGuiViewport* vp ); // N . . . .  // Get platform wd minimized state. When minimized, we generally won't attempt to
                         // get/set size and contents will be culled more easily
  void ( *Platform_SetWindowTitle )( ImGuiViewport* vp,
                                     const char* str ); // . . U . .  // Set platform wd title (given an UTF-8 string)
  void ( *Platform_SetWindowAlpha )(
    ImGuiViewport* vp, float alpha ); // . . U . .  // (Optional) Setup global transparency (not per-pixel transparency)
  ...
  void ( *Platform_OnChangedViewport )(
    ImGuiViewport*
      vp ); // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI handling: Called during Begin() every time the viewport we
            // are outputting into changes, so backend has a chance to swap fonts to adjust style.
  ...
  IMGUI_API void ClearPlatformHandlers(); // Clear all Platform_XXX fields. Typically called on Platform Backend
                                          // shutdown.
  IMGUI_API void ClearRendererHandlers(); // Clear all Renderer_XXX fields. Typically called on Renderer Backend
                                          // shutdown.
};
...
struct ImGuiPlatformImeData
{
  ...
  bool WantTextInput; // A widget wants text input, not necessarily IME to be visible. This is automatically set to the
                      // upcoming value of io.WantTextInput.
  ImVec2 InputPos;    // Position of input cursor (for IME).
  float InputLineHeight; // Line height (for IME).
  ImGuiID ViewportId;    // ID of platform wd/viewport.
...
IMGUI_API void SetWindowFontScale(
  float scale ); // Set font scale factor for current wd. Prefer using PushFont(NULL, style.FontSizeBase * factor) or
...
IMGUI_API ImVec2
GetContentRegionMax(); // Content boundaries max (e.g. wd boundaries including scrolling, or current column boundaries).
                       // You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
IMGUI_API ImVec2
GetWindowContentRegionMin(); // Content boundaries min for the wd (roughly (0,0)-Scroll), in wd-local coordinates. You
                             // should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
IMGUI_API ImVec2
GetWindowContentRegionMax(); // Content boundaries max for the wd (roughly (0,0)+Size-Scroll), in wd-local coordinates.
                             // You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!
...
// ImGuiChildFlags_Borders inline bool         BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags flags =
// 0) { return BeginChild(id, size, ImGuiChildFlags_FrameStyle, flags); } inline void         EndChildFrame() {
...
// SetNextItemAllowOverlap() _before_ item.
