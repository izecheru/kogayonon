...
IMGUI_API bool IsWindowHovered(ImGuiHoveredFlags flags = 0); // is current window hovered and hoverable (e.g. not

// blocked by a popup/modal)? See ImGuiHoveredFlags_ for
...
IMGUI_API void SetWindowSize(const ImVec2& size,
                             ImGuiCond cond = 0); // (not recommended) set current window size - call within

// Begin()/End(). set to ImVec2(0, 0) to force an
...
// Selectable() is clicked is returns true
...
// the contents region). pass -1 to use
...
// tab/window ahead (useful to reduce
...
// Strings starting with '_' are reserved for dear
...
IMGUI_API ImVec2 GetMouseDragDelta(ImGuiMouseButton button = 0,
                                   float lock_threshold = -1.0f); // return the delta from the initial clicking position
...
struct ImGuiIO
{
    ...
    // OS windows.
    bool ConfigViewportPlatformFocusSetsImGuiFocus; //= true // When a platform window is focused (e.g. using Alt+Tab,

    // clicking Platform Title Bar), apply corresponding focus on imgui
    // windows (may clear focus/active id from imgui windows location in
    // other platform windows). In principle this is better enabled but
    // we provide an opt-out, because some Linux window managers tend to
...
struct ImGuiPlatformIO
{
    ...
    void (*Platform_OnChangedViewport)(ImGuiViewport* vp); // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI handling:

    // Called during Begin() every time the
