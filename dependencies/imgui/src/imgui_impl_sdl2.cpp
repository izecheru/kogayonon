...
//  #8558) 2025-04-09: Don't attempt to call SDL_CaptureMouse() on drivers where we don't call
//  SDL_GetGlobalMouseState(). (#8561) 2025-03-21: Fill gamepad inputs and set ImGuiBackendFlags_HasGamepad regardless
//  of ImGuiConfigFlags_NavEnableGamepad being set. 2025-03-10: When dealing with OEM keys, use scancodes instead of
//  translated keycodes to choose ImGuiKey values. (#7136, #7201, #7206, #7306, #7670, #7672, #8468) 2025-02-26: Only
//  start SDL_CaptureMouse() when mouse is being dragged, to mitigate issues with e.g.Linux debuggers not claiming
//  capture back. (#6410, #3650) 2025-02-25: [Docking] Revert to use SDL_GetDisplayBounds() for WorkPos/WorkRect if
//  SDL_GetDisplayUsableBounds() failed. 2025-02-24: Avoid calling SDL_GetGlobalMouseState() when mouse is in relative
//  mode. 2025-02-21: [Docking] Update monitors and work areas information every frame, as the later may change
//  regardless of monitor changes. (#8415) 2025-02-18: Added ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse
//  cursor support. 2025-02-10: Using SDL_OpenURL() in platform_io.Platform_OpenInShellFn handler. 2025-01-20: Made
//  ImGui_ImplSDL2_SetGamepadMode(ImGui_ImplSDL2_GamepadMode_Manual) accept an empty array. 2024-10-24: Emscripten: from
//  SDL 2.30.9, SDL_EVENT_MOUSE_WHEEL event doesn't require dividing by 100.0f. 2024-09-09: use
//  SDL_Vulkan_GetDrawableSize() when available. (#7967, #3190) 2024-08-22: moved some OS/backend related function
//  pointers from ImGuiIO to ImGuiPlatformIO:
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event)
{
    ...
    case SDL_KEYUP: {
        ...
        //     SDL_GetScancodeName(event->key.keysym.scancode), event->key.keysym.mod, event->key.windowID, viewport ?
        //     viewport->ID : 0);
        ...
        io.SetKeyEventNativeData(
            ...
            (int)event->key.keysym.scancode); // To support legacy indexing (<1.87 user code). Legacy backend uses
...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer, void* sdl_gl_context)
{
    ...
    // We left the call to ImGui_ImplSDL2_InitMultiViewportSupport() outside of #ifdef to avoid unused-function
    // warnings.
...
static void ImGui_ImplSDL2_UpdateMouseData()
{
    ...
    const bool is_app_focused =
        (focused_window && (bd->Window == focused_window ||
                            ImGui_ImplSDL2_GetViewportForWindowID(SDL_GetWindowID(focused_window)) != NULL));
#else
...
void ImGui_ImplSDL2_NewFrame()
{
    ...
    // Technically speaking testing for 'bd->MouseButtonsDown == 0' would be more rigorous, but testing for payload
...
#if defined(_WIN32) &&                                                                                                 \
    !(defined(WINAPI_FAMILY) && ((defined(WINAPI_FAMILY_APP) && WINAPI_FAMILY == WINAPI_FAMILY_APP) ||                 \
