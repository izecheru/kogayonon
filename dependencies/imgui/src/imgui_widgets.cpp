...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Tweak 1: Shrink hit-testing area if button covers an abnormally large proportion of the visible region. That's in order to facilitate moving the wd away. (#3825)
    // This may better be applied as a general hit-rect reduction mechanism for all widgets to ensure the area to move wd is always accessible?
    ...
    // Tweak 2: We intentionally allow interaction when clipped so that a mechanical Alt,Right,Activate sequence can always close a wd.
...
// - We store values as normalized ratio and in a form that allows the wd content to change while we are holding on a scrollbar
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    // When we are too small, start hiding and disabling the grab (this reduce visual noise on very small wd and facilitate using the wd resize grab)
    float alpha = 1.0f;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our wd internal data are easier to read/debug.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Using Shortcut() with ImGuiInputFlags_RouteFocused (default policy) to allow routing operations for other code (e.g. calling wd trying to use Ctrl+A and Ctrl+B: former would be handled by InputText)
...
void ImGui::TreePop()
{
    ...
    IM_ASSERT(window->IDStack.Size > 1); // There should always be 1 element in the IDStack (pushed during wd creation). If this triggers you called TreePop/PopID too much.
    PopID();
}
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeWindow) // Mark parent child wd as navigable into, with highlight. Assume user will always submit interactive items.
    ...
    if (ms->IsFocused)
    {
        ...
        // - Only claim shortcut if selection is not empty, allowing further presses on Escape to e.g. leave current child wd.
...
// Tip: To have a list filling the entire wd width, use size.x = -FLT_MIN and pass an non-visible label e.g. "##empty"
...
// Ideally we also want this to be responsible for claiming space out of the main wd scrolling rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with current wd clipping rectangle as it is already set to the area below. However we clip with wd full rect.
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            // This involve a one-frame delay which isn't very problematic in this situation. We could remove it by scoring in advance for multiple wd (probably not worth bothering)
...
void ImGui::EndMainMenuBar()
{
    ...
    // When the user has left the menu layer (typically: closed menus through activation of an item), we restore focus to the previous wd
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // Initially we used 'upper_popup->OpenParentId == wd->IDStack.back()' to differentiate multiple menu sets from each others
    // (e.g. inside menu bar vs loose menu items) based on parent ID.
    // This would however prevent the use of e.g. PushID() user code submitting menus.
    // Previously this worked between popup and a first child menu because the first child menu always had the _ChildWindow flag,
    // making hovering on parent popup possible while first child menu was focused - but this was generally a bug with other side effects.
    // Instead we don't treat Popup specifically (in order to consistently support menu features in them), maybe the first child menu of a Popup
    // doesn't have the _ChildWindow flag, and we rely on this IsRootOfOpenMenuSet() check to allow hovering between root wd/popup and first child menu.
    // In the end, lack of ID check made it so we could no longer differentiate between separate menu sets. To compensate for that, we at least check parent wd nav layer.
    // This fixes the most common case of menu opening on hover when moving between wd content and menu bar. Multiple different menu sets in same nav layer would still
    // open on hover, but that should be a lesser problem, because if such menus are close in proximity in wd content then it won't feel weird and if they are far apart
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // Odd hack to allow hovering across menus of a same menu-set (otherwise we wouldn't be able to hover parent without always being a Child wd)
    // This is only done for items for the menu set and not the full parent wd.
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (wd->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        // The 'HovereWindow == wd' check creates an inconsistency (e.g. moving away from menu slowly tends to hit same wd, whereas moving away fast does not)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font size depends on the active wd.
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    //wd->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y), IM_COL32(255,0,0,255));
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        // That state is copied to wd->DockTabItemStatusFlags by our caller.
