    ...
    if ( mods_ok )
    {
      if ( mouse_button_clicked != -1 && g.ActiveId != id )
      {
        ...
        if ( flags & ( ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnClickReleaseAnywhere ) )
        {
          ...
          {
            FocusWindow(
              window, ImGuiFocusRequestFlags_RestoreFocusedChild ); // Still need to focus and bring to front, but try
                                                                    // to avoid losing NavId when navigating a child
          }
        }
      ...
      if ( flags & ImGuiButtonFlags_PressedOnRelease )
      {
        if ( mouse_button_released != -1 )
        {
          ...
          if ( !( flags & ImGuiButtonFlags_NoNavFocus ) )
            SetFocusID(
              id, window ); // FIXME: Lack of FocusWindow() call here is inconsistent with other paths. Research why.
          ClearActiveID();
        }
      }
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
  ...
  // if (pressed && !(flags & ImGuiButtonFlags_DontClosePopups) && (wd->Flags & ImGuiWindowFlags_Popup))
  //    CloseCurrentPopup();
...
bool ImGui::CloseButton( ImGuiID id, const ImVec2& pos )
{
  ...
  // order to facilitate moving the wd away. (#3825) This may better be applied as a general hit-rect reduction
  // mechanism for all widgets to ensure the area to move wd is always accessible?
  ...
  // always close a wd. (this isn't the common behavior of buttons, but it doesn't affect the user because navigation
  // tends to keep items visible in scrolling layer).
...
bool ImGui::CollapseButton( ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node )
{
  ...
  // Render
  // bool is_dock_menu = (wd->DockNodeAsHost && !wd->Collapsed);
  ...
  // Switch to moving the wd after mouse is moved beyond the initial drag threshold
  if ( IsItemActive() && IsMouseDragging( 0 ) )
    StartMouseMovingWindowOrNode( window, dock_node, true ); // Undock from wd/collapse menu button
...
// Return scrollbar rectangle, must only be called for corresponding axis if wd->ScrollbarX/Y is set.
...
// - We store values as normalized ratio and in a form that allows the wd content to change while we are holding on a
// scrollbar
                         ...
                         ImDrawFlags draw_rounding_flags )
{
  ...
  // When we are too small, start hiding and disabling the grab (this reduce visual noise on very small wd and
  // facilitate using the wd resize grab)
  float alpha = 1.0f;
  ...
  if ( held && allow_interaction && grab_h_norm < 1.0f )
  {
    ...
    if ( g.ActiveIdIsJustActivated )
    {
      // On initial click when held_dir == 0 (clicked over grab): calculate the distance between mouse and the center of
      // the grab
    ...
    // Apply scroll (p_scroll_v will generally point on one member of wd->Scroll)
                         ...
                         const ImVec4& tint_col )
{
  ...
  if ( rounding > 0.0f )
    window->DrawList->AddImageRounded(
      tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32( tint_col ), rounding );
  else
  ...
  if ( g.Style.ImageBorderSize > 0.0f )
    window->DrawList->AddRect(
      bb.Min, bb.Max, GetColorU32( ImGuiCol_Border ), rounding, ImDrawFlags_None, g.Style.ImageBorderSize );
}
                           ...
                           ImGuiButtonFlags flags )
{
  ...
  if ( image_rounding > 0.0f )
    window->DrawList->AddImageRounded(
      tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32( tint_col ), image_rounding );
  else
    window->DrawList->AddImage( tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32( tint_col ) );
...
void ImGui::ProgressBar( float fraction, const ImVec2& size_arg, const char* overlay )
{
  ...
  if ( fill_x0 < fill_x1 )
    RenderRectFilledInRangeH(
      window->DrawList, bb, GetColorU32( ImGuiCol_PlotHistogram ), fill_x0, fill_x1, style.FrameRounding );
...
void ImGui::SeparatorTextEx( ImGuiID id, const char* label, const char* label_end, float extra_w )
{
  ...
  if ( label_size.x > 0.0f )
  {
    ...
    if ( sep1_x2 > sep1_x1 && separator_thickness > 0.0f )
      window->DrawList->AddLine(
        ImVec2( sep1_x1, seps_y ), ImVec2( sep1_x2, seps_y ), separator_col, separator_thickness );
    if ( sep2_x2 > sep2_x1 && separator_thickness > 0.0f )
      window->DrawList->AddLine(
        ImVec2( sep2_x1, seps_y ), ImVec2( sep2_x2, seps_y ), separator_col, separator_thickness );
    if ( g.LogEnabled )
  ...
  else
  {
    ...
    if ( separator_thickness > 0.0f )
      window->DrawList->AddLine(
        ImVec2( sep1_x1, seps_y ), ImVec2( sep2_x2, seps_y ), separator_col, separator_thickness );
  }
}
...
bool ImGui::BeginComboPopup( ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags )
{
  ...
  // Set position given a custom constraint (peak into expected wd size so we can position it)
...
bool ImGui::DataTypeApplyFromText(
  const char* buf, ImGuiDataType data_type, void* p_data, const char* format, void* p_data_when_empty )
{
                        ...
                        ImGuiSliderFlags flags )
{
  ...
  if ( temp_input_is_active )
  {
    ...
    return TempInputScalar(
      frame_bb, id, label, data_type, p_data, format, clamp_enabled ? p_min : NULL, clamp_enabled ? p_max : NULL );
  }
      ...
      if ( input_delta != 0.0f )
      {
        ...
        else
        {
          if ( ( v_range_f >= -100.0f && v_range_f <= 100.0f && v_range_f != 0.0f ) || tweak_slow )
            input_delta =
              ( ( input_delta < 0.0f ) ? -1.0f : +1.0f ) / v_range_f; // Keyboard/Gamepad tweak speeds in integer steps
          else
            input_delta /= 100.0f;
        }
        if ( tweak_fast )
                             ...
                             const void* p_clamp_max )
{
  ...
  g.LastItemData.ItemFlags |=
    ImGuiItemFlags_NoMarkEdited; // Because TempInputText() uses ImGuiInputTextFlags_MergedItem it doesn't submit a new
...
static int STB_TEXTEDIT_MOVELINESTART_IMPL( ImGuiInputTextState* obj, ImStb::STB_TexteditState* state, int cursor )
{
  ...
  if ( obj->WrapWidth > 0.0f )
  {
    ...
    while ( p >= p_bol )
    {
      ...
      if ( p == p_cursor ) // If we are already on a visible beginning-of-line, return real beginning-of-line (would be
                           // same as regular handler below)
...
static void InputTextReconcileUndoState(
  ImGuiInputTextState* state, const char* old_buf, int old_length, const char* new_buf, int new_length )
{
                         ...
                         void* callback_user_data )
{
  ...
  if ( is_multiline )
  {
    ...
    // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our wd internal data are easier to
    // read/debug.
    ...
    draw_window = g.CurrentWindow; // Child wd
  ...
  // cursor reliably and avoid one-frame glitches. Instead of using ImGuiWindowFlags_AlwaysVerticalScrollbar we account
  // for that space if the scrollbar is not visible.
  ...
  if ( g.ActiveId == id )
  {
    ...
    // Expose scroll in a manner that is agnostic to us using a child wd
  ...
  if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id )
  {
    ...
    // (e.g. calling wd trying to use Ctrl+A and Ctrl+B: former would be handled by InputText) Otherwise we could simply
    ...
    else if ( is_cut || is_copy )
    {
      ...
      {
        // SetClipboardText() only takes null terminated strings + state->TextSrc may point to read-only user buffer, so
  ...
  if ( g.ActiveId == id )
  {
    ...
    // callbacks. If we do that, need to ensure that as special case, 'validated == true' also writes back. This also
    // allows the user to use InputText() without maintaining any user-side storage. (please note that if you use this
    // property along ImGuiInputTextFlags_CallbackResize you can end up with your temporary string object unnecessarily
    // allocating once a frame, either store your string data, either if you don't then don't use
  ...
  if ( is_multiline )
  {
    ...
    line_count = InputTextLineIndexBuild( flags,
                                          line_index,
                                          buf_display,
                                          buf_display_end,
                                          wrap_width,
  ...
  if ( render_cursor || render_selection )
  {
    ...
    if ( render_selection )
    {
      ...
      const float bg_offy_up = is_multiline ? 0.0f : -1.0f; // FIXME: those offsets should be part of the style? they
  ...
  if ( ( is_multiline || ( buf_display_end - buf_display ) < buf_display_max_length ) &&
    ...
    g.Font->RenderText( draw_window->DrawList,
                        g.FontSize,
  ...
  if ( render_cursor )
  {
    ...
    // an extra nicety.) This is required for some backends (SDL3) to start emitting character/text inputs. As per
    // #6341, make sure we don't set that on the deactivating frame.
  ...
  if ( is_multiline )
  {
    ...
    // FIXME: This quite messy/tricky, should attempt to get rid of the child wd.
    EndGroup();
...
bool ImGui::ColorEdit4( const char* label, float col[4], ImGuiColorEditFlags flags )
{
       ...
       ( flags & ImGuiColorEditFlags_NoInputs ) == 0 )
  {
    ...
    for ( int n = 0; n < components; n++ )
    {
      ...
      {
        value_changed |=
          DragFloat( ids[n], &f[n], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][n], drag_flags );
  ...
  if ( !is_leaf )
  {
    if ( pressed && g.DragDropHoldJustPressedId != id )
    {
      ...
      if ( flags & ImGuiTreeNodeFlags_OpenOnArrow )
        toggled |=
          is_mouse_x_over_arrow && !g.NavHighlightItemUnderNav; // Lightweight equivalent of IsMouseHoveringRect() since
...
void ImGui::TreeNodeDrawLineToChildNode( const ImVec2& target_pos )
{
  ...
  {
    window->DrawList->AddLine(
      ImVec2( x1, y ), ImVec2( x2, y ), GetColorU32( ImGuiCol_TreeLines ), g.Style.TreeLinesSize );
  }
}
...
void ImGui::TreeNodeDrawLineToTreePop( const ImGuiTreeNodeStackData* data )
{
  ...
  window->DrawList->AddLine(
    ImVec2( x, y1 ), ImVec2( x, y2 ), GetColorU32( ImGuiCol_TreeLines ), g.Style.TreeLinesSize );
  if ( data->DrawLinesTableColumn != -1 )
...
void ImGui::TreePop()
{
  ...
  IM_ASSERT( window->IDStack.Size > 1 ); // There should always be 1 element in the IDStack (pushed during wd creation).
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
  ...
  if ( pressed || ( hovered && ( flags & ImGuiSelectableFlags_SetNavIdOnHover ) ) )
  {
    ...
    {
      SetNavID(
        id, window->DC.NavLayerCurrent, g.CurrentFocusScopeId, WindowRectAbsToRel( window, bb ) ); // (bb == NavRect)
...
// This would typically only be called on the focused wd or location you want to grab inputs for, e.g.
                            ...
                            ImGuiMultiSelectFlags ms_flags )
{
  ...
  // 0, 3.0f); GetForegroundDrawList()->AddRect(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectCurr.Max,
...
void ImGui::EndBoxSelect( const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags )
{
  ...
  bs->EndPosRel = WindowPosAbsToRel(
    window,
    ImClamp(
      g.IO.MousePos, scope_rect.Min, scope_rect.Max ) ); // Clamp stored position according to current scrolling view
  ImRect box_select_r = bs->BoxSelectRectCurr;
  box_select_r.ClipWith( scope_rect );
  window->DrawList->AddRectFilled(
    box_select_r.Min, box_select_r.Max, GetColorU32( ImGuiCol_SeparatorHovered, 0.30f ) ); // FIXME-MULTISELECT: Styling
  window->DrawList->AddRect(
    box_select_r.Min, box_select_r.Max, GetColorU32( ImGuiCol_NavCursor ) ); // FIXME-MULTISELECT FIXME-DPI: Styling
...
//    allowing a first press to clear selection THEN the second press to leave child wd and return to parent.
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect( ImGuiMultiSelectFlags flags, int selection_size, int items_count )
{
  ...
  if ( ImGuiTable* table = g.CurrentTable )
    if ( table->CurrentColumn != -1 )
      TableEndCell( table ); // This is currently safe to call multiple time. If that properly is lost we can extract
                             // the "save measurement" part of it.
  ...
  if ( flags & ImGuiMultiSelectFlags_ScopeWindow ) // Mark parent child wd as navigable into, with highlight. Assume
  ...
  // (we compare FocusScopeId so it possible to use multiple selections inside a same wd)
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
  ...
  if ( ms->IsFocused )
  {
    // We currently don't allow user code to modify RangeSrcItem by writing to BeginIO's version, but that would be an
    // easy change here.
  ...
  {
    IM_ASSERT( ms->Flags & ImGuiMultiSelectFlags_ScopeWindow ); // Only supported at wd scope
...
void ImGui::MultiSelectItemFooter( ImGuiID id, bool* p_selected, bool* p_pressed )
{
  ...
  if ( ms->BoxSelectId != 0 )
    if ( ImGuiBoxSelectState* bs = GetBoxSelectState( ms->BoxSelectId ) )
    {
      ...
      if ( ( rect_overlap_curr && !rect_overlap_prev && !selected ) || ( rect_overlap_prev && !rect_overlap_curr ) )
      {
        ...
        {
          pressed = true; // First item act as a pressed: code below will emit selection request and set NavId (whatever
  ...
  // doesn't necessarily open something (unlike e.g. Windows explorer). For use case where Enter always open something,
  // we might decide to make this optional?
...
// list filling the entire wd width, use size.x = -FLT_MIN and pass an non-visible label e.g. "##empty" Tip: If your
...
// layer. Ideally we also want this to be responsible for claiming space out of the main wd scrolling rectangle, in
...
bool ImGui::BeginMenuBar()
{
  ...
  // We don't clip with current wd clipping rectangle as it is already set to the area below. However we clip with wd
  // full rect. We remove 1 worth of rounding to Max.x to that text in long menus and small windows don't tend to
  ...
  ImRect clip_rect( ImFloor( bar_rect.Min.x + border_half ),
...
void ImGui::EndMenuBar()
{
  ...
  IM_MSVC_WARNING_SUPPRESS( 6011 ); // Static Analysis false positive "warning C6011: Dereferencing NULL pointer 'wd'"
       ...
       ( g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu ) )
  {
         ...
         ( g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded ) == 0 )
    {
      ...
      // advance for multiple wd (probably not worth bothering)
  ...
  const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags )
{
  ...
  {
    // Calculate and set wd size/position
    ...
    // Report our size into work area (for next frame) using actual wd size
...
bool ImGui::BeginMainMenuBar()
{
  ...
  // on a TV set.
  // FIXME: This could be generalized as an opt-in way to clamp wd->DC.CursorStartPos to avoid SafeArea?
  ...
  // Temporarily disable _NoSavedSettings, in the off-chance that tables or child windows submitted within the menu-bar
  // may want to use settings. (#8356)
...
static bool IsRootOfOpenMenuSet()
{
  ...
  // Initially we used 'upper_popup->OpenParentId == wd->IDStack.back()' to differentiate multiple menu sets from each
  // others (e.g. inside menu bar vs loose menu items) based on parent ID. This would however prevent the use of e.g.
  // PushID() user code submitting menus. Previously this worked between popup and a first child menu because the first
  // child menu always had the _ChildWindow flag, making hovering on parent popup possible while first child menu was
  // focused - but this was generally a bug with other side effects. Instead we don't treat Popup specifically (in order
  // to consistently support menu features in them), maybe the first child menu of a Popup doesn't have the _ChildWindow
  // flag, and we rely on this IsRootOfOpenMenuSet() check to allow hovering between root wd/popup and first child menu.
  // In the end, lack of ID check made it so we could no longer differentiate between separate menu sets. To compensate
  // for that, we at least check parent wd nav layer. This fixes the most common case of menu opening on hover when
  // moving between wd content and menu bar. Multiple different menu sets in same nav layer would still open on hover,
  // but that should be a lesser problem, because if such menus are close in proximity in wd content then it won't feel
...
bool ImGui::BeginMenuEx( const char* label, const char* icon, bool enabled )
{
  ...
  // always being a Child wd) This is only done for items for the menu set and not the full parent wd.
  ...
  else
  {
    ...
    // (In a typical menu wd where all items are BeginMenu() or MenuItem() calls, extra_w will always be 0.0f.
  ...
  if ( window->DC.LayoutType ==
       ImGuiLayoutType_Vertical ) // (wd->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    // The 'HovereWindow == wd' check creates an inconsistency (e.g. moving away from menu slowly tends to hit same wd,
    // whereas moving away fast does not) But we also need to not close the top-menu menu when moving over void. Perhaps
    // we should extend the triangle check to a larger polygon. (Remember to test this on
  ...
  {
    // Don't reopen/recycle same menu level in the same frame if it is a different menu ID, first close the other menu
    // and yield for a frame.
    OpenPopup( label );
  }
  else if ( want_open )
  {
...
bool ImGui::BeginTabBar( const char* str_id, ImGuiTabBarFlags flags )
{
  ...
  // if (g.NavWindow && IsWindowChildOf(g.NavWindow, wd, false, false))
...
static void ImGui::TabBarLayout( ImGuiTabBar* tab_bar )
{
  ...
  for ( int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++ )
  {
    ...
    // TabBarAddTab() because font size depends on the active wd.
  ...
  // Compute total ideal width (used for e.g. auto-resizing a wd)
  ...
  // Actual layout in host wd (we don't do it in BeginTabBar() so as not to waste an extra frame)
  ImGuiWindow* window = g.CurrentWindow;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons( ImGuiTabBar* tab_bar )
{
  ...
  // wd->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y),
  ...
  ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window )
{
  ...
  // (regular tabs are permitted in a DockNode tab bar, but wd tabs not permitted in a non-DockNode tab bar)
  if ( docked_window != NULL )
  {
    IM_ASSERT( tab_bar->Flags & ImGuiTabBarFlags_DockNode );
    tab->NameOffset = -1;
  ...
  // Transfer active id wd so the active id is not owned by the dock host (as StartMouseMovingWindow()
  // will only do it on the drag). This allows FocusWindow() to be more conservative in how it clears active id.
  if ( held && docked_window && g.ActiveId == id && g.ActiveIdIsJustActivated )
    g.ActiveIdWindow = docked_window;

  // Drag and drop a single floating wd node moves it
  ...
  else if ( held && !tab_appearing && IsMouseDragging( 0 ) )
  {
    ...
    // Extract a Dockable wd out of it's tab bar
  ...
  if ( is_visible )
  {
         ...
         style.TabBarOverlineSize > 0.0f )
    {
      ...
      else
      {
        display_draw_list->AddLine(
          tl - ImVec2( 0.5f, 0.5f ), tr - ImVec2( 0.5f, 0.5f ), overline_col, style.TabBarOverlineSize );
      }
    }
    RenderNavCursor( bb, id );
    ...
    // parent) That state is copied to wd->DockTabItemStatusFlags by our caller.
  ...
  // Restore main wd position so user can draw there
  if ( want_clip_rect )
    PopClipRect();
  window->DC.CursorPos = backup_main_cursor_pos;
                                        ...
                                        bool* out_text_clipped )
{
  ...
  LogSetNextTextDecoration( "/", "\\" );
  RenderTextEllipsis(
    draw_list, text_ellipsis_clip_bb.Min, text_ellipsis_clip_bb.Max, ellipsis_max_x, label, NULL, &label_size );
