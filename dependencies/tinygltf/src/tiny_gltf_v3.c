...
static tg3_arena* tg3__arena_create( const tg3_memory_config* config )
{
  ...
  arena->alloc = alloc;
  arena->block_size = ( config && config->arena_block_size > 0 ) ? (size_t)config->arena_block_size
                                                                 : (size_t)TG3__ARENA_DEFAULT_BLOCK_SIZE;
  arena->memory_budget =
    ( config && config->memory_budget > 0 ) ? (size_t)config->memory_budget : (size_t)TINYGLTF3_MAX_MEMORY_BYTES;
  arena->max_single_alloc = ( config && config->max_single_alloc > 0 ) ? (size_t)config->max_single_alloc : 0;
  return arena;
}
...
static tg3__arena_block* tg3__arena_new_block( tg3_arena* arena, size_t min_size )
{
  ...
  if ( cap < min_size )
    cap = min_size;
  if ( arena->max_single_alloc && cap > arena->max_single_alloc )
    return NULL;
...
static void* tg3__arena_alloc( tg3_arena* arena, size_t size )
{
  ...
  if ( !arena || size == 0 )
    return NULL;
  if ( arena->max_single_alloc && size > arena->max_single_alloc )
    return NULL;
...
static void tg3__error_push( tg3_error_stack* es,
                             tg3_severity sev,
                             tg3_error_code code,
                             const char* msg,
                             ...
                             int64_t byte_offset )
{
  ...
  if ( sev == TG3_SEVERITY_ERROR )
    es->has_error = 1;
}

static void tg3__error_pushf( tg3_error_stack* es,
                              tg3_arena* arena,
                              tg3_severity sev,
                              tg3_error_code code,
                              const char* json_path,
...
TINYGLTF3_API int32_t tg3_accessor_byte_stride( const tg3_accessor* accessor, const tg3_buffer_view* bv )
{
  int32_t comp;
  int32_t num;
  if ( bv && bv->byte_stride > 0 )
    return (int32_t)bv->byte_stride;
...
static int tg3__parse_data_uri( const char* uri, uint32_t uri_len, tg3__data_uri_result* result )
{
  ...
  const char* semi;
  size_t mime_len;
  if ( !uri || uri_len < 5u || TINYGLTF3_MEMCMP( uri, "data:", 5 ) != 0 )
    return 0;
  p = uri + 5;
  end = uri + uri_len;
  semi = p;
  ...
  if ( mime_len >= sizeof( result->mime_type ) )
    mime_len = sizeof( result->mime_type ) - 1u;
  TINYGLTF3_MEMCPY( result->mime_type, p, mime_len );
  result->mime_type[mime_len] = '\0';
  p = semi + 1;
  if ( (size_t)( end - p ) < 7u || TINYGLTF3_MEMCMP( p, "base64,", 7 ) != 0 )
    return 0;
  p += 7;
  result->data_start = p;
...
static uint8_t* tg3__decode_data_uri(
  tg3_arena* arena, const char* uri, uint32_t uri_len, size_t* out_len, char* out_mime, size_t out_mime_cap )
{
  ...
  if ( out_mime && out_mime_cap > 0 )
  {
    ...
    if ( mlen >= out_mime_cap )
      mlen = out_mime_cap - 1u;
...
static int32_t tg3__fs_read_file(
  uint8_t** out_data, uint64_t* out_size, const char* path, uint32_t path_len, void* ud )
{
  FILE* fp;
  long size;
  uint8_t* data;
...
static int32_t tg3__fs_write_file( const char* path, uint32_t path_len, const uint8_t* data, uint64_t size, void* ud )
{
  FILE* fp;
  size_t nwritten;
  (void)path_len;
...
{
  if ( !fs->read_file )
    fs->read_file = tg3__fs_read_file;
  if ( !fs->free_file )
    fs->free_file = tg3__fs_free_file;
  if ( !fs->file_exists )
    fs->file_exists = tg3__fs_file_exists;
  if ( !fs->write_file )
    fs->write_file = tg3__fs_write_file;
}
#endif
...
static int tg3__json_number_to_int32( const tg3json_value* v, int32_t* out )
{
  double real;
  int32_t converted;
  if ( !tg3__json_is_number( v ) || !out )
    return 0;
  if ( v->type == TG3JSON_INT )
  {
    if ( v->u.integer < INT32_MIN || v->u.integer > INT32_MAX )
      return 0;
...
static double tg3__json_number_to_double( const tg3json_value* v )
{
  ...
  if ( v->type == TG3JSON_REAL )
    return v->u.real;
  return 0.0;
}

static int tg3__json_is_object( const tg3json_value* v )
{
  return v && v->type == TG3JSON_OBJECT;
}

static int tg3__json_is_array( const tg3json_value* v )
{
  return v && v->type == TG3JSON_ARRAY;
}
...
static int tg3__parse_string(
  tg3__parse_ctx* ctx, const tg3json_value* o, const char* key, tg3_str* out, int required, const char* parent )
{
  ...
  if ( !it )
  {
    if ( required )
    {
      tg3__error_pushf( ctx->errors,
                        ctx->arena,
                        TG3_SEVERITY_ERROR,
                        TG3_ERR_JSON_MISSING_FIELD,
                        parent,
...
static int tg3__parse_int(
  tg3__parse_ctx* ctx, const tg3json_value* o, const char* key, int32_t* out, int required, const char* parent )
{
  const tg3json_value* it = tg3__json_get( o, key );
  if ( !it )
  {
  ...
  if ( !tg3__json_number_to_int32( it, out ) )
  {
    tg3__error_pushf( ctx->errors,
                      ...
                      TG3_ERR_JSON_TYPE_MISMATCH,
                      parent,
                      "Field '%s' must be a finite int32-valued number",
                      key );
    return 0;
  }
  return 1;
...
static int tg3__parse_uint64(
  tg3__parse_ctx* ctx, const tg3json_value* o, const char* key, uint64_t* out, int required, const char* parent )
{
  const tg3json_value* it = tg3__json_get( o, key );
  if ( !it )
  {
  ...
  if ( !tg3__json_number_to_uint64( it, out ) )
  {
    tg3__error_pushf( ctx->errors,
                      ...
                      TG3_ERR_JSON_TYPE_MISMATCH,
                      parent,
                      "Field '%s' must be a finite uint64-valued number",
                      key );
    return 0;
  }
  return 1;
...
static int tg3__parse_double(
  tg3__parse_ctx* ctx, const tg3json_value* o, const char* key, double* out, int required, const char* parent )
{
  const tg3json_value* it = tg3__json_get( o, key );
  if ( !it )
  {
...
static int tg3__parse_number_array( tg3__parse_ctx* ctx,
                                    const tg3json_value* o,
                                    const char* key,
                                    const double** out,
                                    uint32_t* out_count,
                                    int required,
                                    const char* parent )
{
  ...
  {
    tg3__error_push(
      ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM allocating number array", parent, -1 );
    return 0;
  }
  for ( i = 0; i < count; ++i )
  {
...
static int tg3__parse_int_array( tg3__parse_ctx* ctx,
                                 const tg3json_value* o,
                                 const char* key,
                                 const int32_t** out,
                                 uint32_t* out_count,
                                 ...
                                 const char* parent )
{
  ...
  if ( !arr )
  {
    tg3__error_push( ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM allocating int array", parent, -1 );
    return 0;
  }
  for ( i = 0; i < count; ++i )
  {
...
static void tg3__parse_number_to_fixed( const tg3json_value* o, const char* key, double* out, uint32_t max_count )
{
  const tg3json_value* it = tg3__json_get( o, key );
  size_t count;
  size_t i;
...
static int tg3__parse_string_array( tg3__parse_ctx* ctx,
                                    const tg3json_value* o,
                                    const char* key,
                                    const tg3_str** out,
                                    uint32_t* out_count,
    ...
    if ( j->u.array.count > 0 )
    {
      ...
      if ( arr )
      {
        for ( i = 0; i < j->u.array.count; ++i )
          arr[i] = tg3__json_to_value( ctx, &j->u.array.items[i] );
        v.array_data = arr;
  ...
  case TG3JSON_OBJECT:
    ...
    if ( j->u.object.count > 0 )
    {
      ...
      if ( pairs )
      {
        ...
        {
          pairs[i].key = tg3__arena_str( ctx->arena, j->u.object.items[i].key, (uint32_t)j->u.object.items[i].key_len );
...
static void tg3__parse_extras_and_extensions( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_extras_ext* ee )
{
...
{
  TINYGLTF3_MEMSET( ti, 0, sizeof( *ti ) );
  ti->index = -1;
}
...
static int tg3__parse_texture_info( tg3__parse_ctx* ctx, const tg3json_value* o, const char* key, tg3_texture_info* ti )
{
  const tg3json_value* it = tg3__json_get( o, key );
...
static int tg3__parse_normal_texture_info( tg3__parse_ctx* ctx,
                                           const tg3json_value* o,
                                           const char* key,
...
{
  if ( len == 6 && TINYGLTF3_MEMCMP( s, "SCALAR", 6 ) == 0 )
    return TG3_TYPE_SCALAR;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "VEC2", 4 ) == 0 )
    return TG3_TYPE_VEC2;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "VEC3", 4 ) == 0 )
    return TG3_TYPE_VEC3;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "VEC4", 4 ) == 0 )
    return TG3_TYPE_VEC4;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "MAT2", 4 ) == 0 )
    return TG3_TYPE_MAT2;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "MAT3", 4 ) == 0 )
    return TG3_TYPE_MAT3;
  if ( len == 4 && TINYGLTF3_MEMCMP( s, "MAT4", 4 ) == 0 )
    return TG3_TYPE_MAT4;
  return -1;
}

static int tg3__parse_accessor_sparse( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_accessor_sparse* sparse )
{
...
static int tg3__uri_is_safe( const char* uri, uint32_t uri_len )
{
  ...
  if ( TINYGLTF3_MEMCHR( uri, '\0', uri_len ) != NULL )
    return 0;
  /* Reject absolute paths (POSIX and Windows). */
  if ( uri[0] == '/' || uri[0] == '\\' )
    return 0;
  /* Reject Windows drive prefixes like "C:". */
  if ( uri_len >= 2 && uri[1] == ':' && ( ( uri[0] >= 'A' && uri[0] <= 'Z' ) || ( uri[0] >= 'a' && uri[0] <= 'z' ) ) )
  {
    return 0;
  }
...
static int tg3__load_external_file(
  tg3__parse_ctx* ctx, uint8_t** out_data, uint64_t* out_size, const char* uri, uint32_t uri_len )
{
  char path_buf[4096];
  uint32_t path_len = 0;
  int32_t ok;
  ...
  {
    tg3__error_push(
      ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_FS_NOT_AVAILABLE, "No filesystem callbacks available", NULL, -1 );
    return 0;
  }
  if ( !tg3__uri_is_safe( uri, uri_len ) )
  {
    tg3__error_push( ctx->errors,
                     TG3_SEVERITY_ERROR,
                     TG3_ERR_INVALID_VALUE,
  ...
  if ( uri_len >= sizeof( path_buf ) )
    return 0;
  if ( ctx->base_dir_len > 0 )
  {
    if ( ctx->base_dir_len >= sizeof( path_buf ) - uri_len - 1u )
      return 0;
  ...
  if ( path_len + uri_len >= sizeof( path_buf ) )
    return 0;
  ...
  {
    tg3__error_pushf(
      ctx->errors, ctx->arena, TG3_SEVERITY_ERROR, TG3_ERR_FILE_READ, NULL, "Failed to read file: %s", path_buf );
    return 0;
  }
  if ( ctx->opts.max_external_file_size > 0 && *out_size > ctx->opts.max_external_file_size )
  {
...
static int tg3__parse_buffer( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_buffer* buf, int32_t buf_idx )
{
  ...
  if ( ctx->is_binary && buf_idx == 0 && buf->uri.len == 0 )
  {
    ...
    if ( !tg3__u64_fits_size( byte_length ) )
    {
      tg3__error_push( ctx->errors,
                       TG3_SEVERITY_ERROR,
                       TG3_ERR_OUT_OF_MEMORY,
                       "buffer.byteLength exceeds addressable size",
                       NULL,
                       -1 );
      return 0;
    }
    ...
    {
      tg3__error_push( ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM for buffer data", NULL, -1 );
      return 0;
    }
    if ( byte_length > 0 )
      TINYGLTF3_MEMCPY( data, ctx->bin_data, (size_t)byte_length );
    buf->data.data = data;
    buf->data.count = byte_length;
  }
  else if ( buf->uri.len > 0 )
  {
    if ( tg3_is_data_uri( buf->uri.data, buf->uri.len ) )
    {
      ...
      {
        tg3__error_push(
          ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_DATA_URI_DECODE, "Failed to decode buffer data URI", NULL, -1 );
        return 0;
      }
      ...
      if ( tg3__load_external_file( ctx, &file_data, &file_size, buf->uri.data, buf->uri.len ) )
      {
        ...
        if ( !tg3__u64_fits_size( file_size ) )
        {
          tg3__error_push( ctx->errors,
                           TG3_SEVERITY_ERROR,
                           TG3_ERR_OUT_OF_MEMORY,
                           "external buffer exceeds addressable size",
                           NULL,
                           -1 );
          if ( ctx->opts.fs.free_file )
            ctx->opts.fs.free_file( file_data, file_size, ctx->opts.fs.user_data );
          tg3__parse_extras_and_extensions( ctx, o, &buf->ext );
          return 0;
        }
        ...
        {
          tg3__error_push(
            ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM for external buffer data", NULL, -1 );
        }
        if ( ctx->opts.fs.free_file )
          ctx->opts.fs.free_file( file_data, file_size, ctx->opts.fs.user_data );
      }
...
static int tg3__parse_buffer_view( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_buffer_view* bv )
{
  ...
  if ( stride != 0 && ( stride < 4 || stride > 252 ) )
  {
    tg3__error_pushf( ctx->errors,
                      ...
                      TG3_ERR_INVALID_VALUE,
                      "/bufferView",
                      "byteStride %d must be 0 or in [4, 252]",
                      stride );
    return 0;
  }
...
static int tg3__parse_material( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_material* mat )
{
  ...
  if ( tg3__json_is_object( pbr_it ) )
  {
    ...
    tg3__parse_texture_info(
      ctx, pbr_it, "metallicRoughnessTexture", &mat->pbr_metallic_roughness.metallic_roughness_texture );
  ...
  if ( tg3__json_is_array( targets_it ) )
  {
    ...
    if ( tcount > 0 )
    {
      ...
      const tg3_str_int_pair** target_arrays =
        (const tg3_str_int_pair**)tg3__arena_alloc( ctx->arena, tcount * sizeof( tg3_str_int_pair* ) );
...
static int tg3__parse_mesh( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_mesh* mesh )
{
  ...
  if ( tg3__json_is_array( prim_it ) )
  {
    ...
    if ( count > 0 )
    {
      ...
      {
        for ( i = 0; i < count; ++i )
          tg3__parse_primitive( ctx, tg3json_array_get( prim_it, i ), &prims[i] );
...
static int tg3__parse_node( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_node* node )
{
  ...
  if ( tg3__json_is_object( ext_it ) )
  {
    ...
    if ( tg3__json_is_object( khr_lights ) )
      tg3__parse_int( ctx, khr_lights, "light", &node->light, 0, "/node/extensions/KHR_lights_punctual" );
    if ( tg3__json_is_object( khr_audio ) )
      tg3__parse_int( ctx, khr_audio, "emitter", &node->emitter, 0, "/node/extensions/KHR_audio" );
    if ( tg3__json_is_object( msft_lod ) )
      tg3__parse_int_array( ctx, msft_lod, "ids", &node->lods, &node->lods_count, 0, "/node/extensions/MSFT_lod" );
  }
...
static int tg3__parse_animation( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_animation* anim )
{
  ...
  if ( tg3__json_is_array( ch_it ) )
  {
    ...
    {
      size_t i;
      tg3_animation_channel* channels =
        (tg3_animation_channel*)tg3__arena_alloc( ctx->arena, count * sizeof( tg3_animation_channel ) );
      if ( channels )
      {
        for ( i = 0; i < count; ++i )
        {
  ...
  if ( tg3__json_is_array( samp_it ) )
  {
    ...
    {
      size_t i;
      tg3_animation_sampler* samplers =
        (tg3_animation_sampler*)tg3__arena_alloc( ctx->arena, count * sizeof( tg3_animation_sampler ) );
      if ( samplers )
      {
        for ( i = 0; i < count; ++i )
        {
...
static int tg3__parse_scene( tg3__parse_ctx* ctx, const tg3json_value* o, tg3_scene* scene )
{
  ...
  if ( tg3__json_is_object( ext_it ) )
  {
    ...
    {
      tg3__parse_int_array( ctx,
                            audio_it,
                            "emitters",
                            &scene->audio_emitters,
                            &scene->audio_emitters_count,
                            0,
...
#define TG3__STREAM_CB( cb_name, model_arr, model_cnt )                                                                \
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        tg3_stream_action _sa =                                                                                        \
          ( ctx )->opts.stream->cb_name( &( model_arr )[_si], (int32_t)_si, ( ctx )->opts.stream->user_data );         \
        if ( _sa == TG3_STREAM_ABORT )                                                                                 \
...
#define TG3__IDX_BAD( path_str, fmt, ... )                                                                             \
  do                                                                                                                   \
  {                                                                                                                    \
    if ( errs < TG3__IDX_ERR_CAP )                                                                                     \
    {                                                                                                                  \
      tg3__error_pushf(                                                                                                \
        ctx->errors, ctx->arena, TG3_SEVERITY_ERROR, TG3_ERR_INVALID_INDEX, ( path_str ), fmt, __VA_ARGS__ );          \
    }                                                                                                                  \
    ++errs;                                                                                                            \
  } while ( 0 )
#define TG3__CHECK_REQ( idx, max, path_str, fmt, ... )                                                                 \
  do                                                                                                                   \
  {                                                                                                                    \
...
#define TG3__CHECK_OPT( idx, max, path_str, fmt, ... )                                                                 \
  do                                                                                                                   \
  {                                                                                                                    \
  ...
  {
    TG3__CHECK_OPT( m->default_scene,
                    m->scenes_count,
                    "/scene",
  ...
  {
    TG3__CHECK_REQ( m->buffer_views[i].buffer,
                    m->buffers_count,
                    "/bufferViews",
                    "bufferViews[%u].buffer %d out of range [0,%u)",
                    i,
                    m->buffer_views[i].buffer,
                    m->buffers_count );
  }
  for ( i = 0; i < m->accessors_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    const tg3_accessor* a = &m->accessors[i];
    TG3__CHECK_OPT( a->buffer_view,
                    m->buffer_views_count,
                    "/accessors",
    ...
    {
      TG3__CHECK_REQ( a->sparse.indices.buffer_view,
                      m->buffer_views_count,
                      "/accessors",
                      "accessors[%u].sparse.indices.bufferView %d out of range [0,%u)",
                      i,
                      a->sparse.indices.buffer_view,
                      m->buffer_views_count );
      TG3__CHECK_REQ( a->sparse.values.buffer_view,
                      m->buffer_views_count,
                      "/accessors",
                      "accessors[%u].sparse.values.bufferView %d out of range [0,%u)",
                      i,
                      a->sparse.values.buffer_view,
                      m->buffer_views_count );
    }
  }
  for ( i = 0; i < m->meshes_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    ...
    for ( j = 0; j < me->primitives_count && errs < TG3__IDX_ERR_CAP; ++j )
    {
      ...
      {
        TG3__CHECK_REQ( p->attributes[ai].value,
                        m->accessors_count,
                        "/meshes",
  ...
  for ( i = 0; i < m->nodes_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    ...
    uint32_t k;
    TG3__CHECK_OPT(
      n->mesh, m->meshes_count, "/nodes", "nodes[%u].mesh %d out of range [0,%u)", i, n->mesh, m->meshes_count );
    TG3__CHECK_OPT(
      n->skin, m->skins_count, "/nodes", "nodes[%u].skin %d out of range [0,%u)", i, n->skin, m->skins_count );
    TG3__CHECK_OPT( n->camera,
                    m->cameras_count,
                    "/nodes",
                    "nodes[%u].camera %d out of range [0,%u)",
                    i,
                    n->camera,
                    m->cameras_count );
    TG3__CHECK_OPT(
      n->light, m->lights_count, "/nodes", "nodes[%u].light %d out of range [0,%u)", i, n->light, m->lights_count );
    for ( k = 0; k < n->children_count && errs < TG3__IDX_ERR_CAP; ++k )
    {
      TG3__CHECK_REQ( n->children[k],
                      m->nodes_count,
                      "/nodes",
  ...
  {
    TG3__CHECK_OPT( m->textures[i].source,
                    m->images_count,
                    "/textures",
                    "textures[%u].source %d out of range [0,%u)",
                    i,
                    m->textures[i].source,
                    m->images_count );
    TG3__CHECK_OPT( m->textures[i].sampler,
                    m->samplers_count,
                    "/textures",
                    "textures[%u].sampler %d out of range [0,%u)",
                    i,
                    m->textures[i].sampler,
                    m->samplers_count );
  }
  for ( i = 0; i < m->images_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    TG3__CHECK_OPT( m->images[i].buffer_view,
                    m->buffer_views_count,
                    "/images",
                    "images[%u].bufferView %d out of range [0,%u)",
                    i,
                    m->images[i].buffer_view,
                    m->buffer_views_count );
  }
  for ( i = 0; i < m->skins_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    ...
    uint32_t k;
    TG3__CHECK_OPT( s->inverse_bind_matrices,
                    m->accessors_count,
                    "/skins",
  ...
  for ( i = 0; i < m->animations_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    ...
    {
      TG3__CHECK_REQ( an->channels[k].sampler,
                      an->samplers_count,
                      "/animations",
                      "animations[%u].channels[%u].sampler %d out of range [0,%u)",
                      i,
                      k,
                      an->channels[k].sampler,
                      an->samplers_count );
      TG3__CHECK_OPT( an->channels[k].target.node,
                      m->nodes_count,
                      "/animations",
                      "animations[%u].channels[%u].target.node %d out of range [0,%u)",
                      i,
                      k,
                      an->channels[k].target.node,
                      m->nodes_count );
    }
    for ( k = 0; k < an->samplers_count && errs < TG3__IDX_ERR_CAP; ++k )
    {
      TG3__CHECK_REQ( an->samplers[k].input,
                      m->accessors_count,
                      "/animations",
                      ...
                      an->samplers[k].input,
                      m->accessors_count );
      TG3__CHECK_REQ( an->samplers[k].output,
                      m->accessors_count,
                      "/animations",
                      "animations[%u].samplers[%u].output %d out of range [0,%u)",
                      i,
                      k,
                      an->samplers[k].output,
                      m->accessors_count );
    }
  }
  for ( i = 0; i < m->scenes_count && errs < TG3__IDX_ERR_CAP; ++i )
  {
    ...
    {
      TG3__CHECK_REQ( s->audio_emitters[k],
                      m->audio_emitters_count,
                      "/scenes",
...
{
  return component_type == TG3_COMPONENT_TYPE_UNSIGNED_BYTE || component_type == TG3_COMPONENT_TYPE_UNSIGNED_SHORT ||
         component_type == TG3_COMPONENT_TYPE_UNSIGNED_INT;
}

static int tg3__validate_range_in_buffer_view(
  tg3__parse_ctx* ctx, const tg3_buffer_view* bv, uint64_t byte_offset, uint64_t byte_length, const char* what )
{
  uint64_t end;
...
static int tg3__validate_resources( tg3__parse_ctx* ctx, const tg3_model* m )
{
  ...
  for ( i = 0; i < m->buffers_count; ++i )
  {
    ...
    if ( b->data.count > 0 && b->byte_length > b->data.count )
    {
      tg3__error_pushf( ctx->errors,
                        ...
                        i,
                        (unsigned long long)b->byte_length,
  ...
  for ( i = 0; i < m->buffer_views_count; ++i )
  {
    ...
    if ( tg3__u64_add_overflow( bv->byte_offset, bv->byte_length, &end ) || end > buffer_size )
    {
      tg3__error_pushf( ctx->errors,
                        ctx->arena,
                        TG3_SEVERITY_ERROR,
  ...
  for ( i = 0; i < m->accessors_count; ++i )
  {
    ...
    if ( tg3__u64_mul_overflow( (uint64_t)comp_size, (uint64_t)num_comp, &elem_size ) )
    {
      tg3__error_pushf( ctx->errors,
                        ...
                        TG3_ERR_INVALID_ACCESSOR,
                        "/accessors",
                        "accessors[%u] element size overflows",
                        i );
      ok = 0;
      continue;
    }
    if ( a->buffer_view >= 0 && (uint32_t)a->buffer_view < m->buffer_views_count )
    {
      ...
      uint64_t span = 0;
      if ( stride < elem_size || ( a->count > 0 && ( tg3__u64_mul_overflow( a->count - 1u, stride, &last_offset ) ||
                                                     tg3__u64_add_overflow( last_offset, elem_size, &span ) ) ) )
      {
        tg3__error_pushf( ctx->errors,
                          ...
                          TG3_ERR_INVALID_ACCESSOR,
                          "/accessors",
                          "accessors[%u] byte stride/count overflows",
                          i );
        ok = 0;
      }
      else if ( !tg3__validate_range_in_buffer_view( ctx, bv, a->byte_offset, a->count > 0 ? span : 0, "accessor" ) )
      {
        ok = 0;
      }
    }
    if ( a->sparse.is_sparse )
    {
      ...
      if ( !tg3__valid_sparse_index_component_type( a->sparse.indices.component_type ) )
      {
        tg3__error_pushf( ctx->errors,
                          ...
                          i,
                          a->sparse.indices.component_type );
        ok = 0;
      }
      if ( a->sparse.indices.buffer_view >= 0 && (uint32_t)a->sparse.indices.buffer_view < m->buffer_views_count )
      {
        ...
        if ( idx_size_i <= 0 || tg3__u64_mul_overflow( sparse_count, (uint64_t)idx_size_i, &bytes ) ||
             !tg3__validate_range_in_buffer_view( ctx, bv, a->sparse.indices.byte_offset, bytes, "sparse indices" ) )
        {
          ok = 0;
        }
      }
      if ( a->sparse.values.buffer_view >= 0 && (uint32_t)a->sparse.values.buffer_view < m->buffer_views_count )
      {
        const tg3_buffer_view* bv = &m->buffer_views[a->sparse.values.buffer_view];
        uint64_t bytes = 0;
        if ( tg3__u64_mul_overflow( sparse_count, elem_size, &bytes ) ||
             !tg3__validate_range_in_buffer_view( ctx, bv, a->sparse.values.byte_offset, bytes, "sparse values" ) )
        {
          ok = 0;
        }
      }
...
static tg3_error_code tg3__parse_from_json( tg3__parse_ctx* ctx, const tg3json_value* json_doc, tg3_model* model )
{
  ...
  {
    tg3__error_push(
      ctx->errors, TG3_SEVERITY_ERROR, TG3_ERR_MISSING_REQUIRED, "Missing required 'asset' property", "/", -1 );
  ...
  if ( ctx->opts.stream && ctx->opts.stream->on_asset )
  {
    ...
    if ( sa == TG3_STREAM_ABORT )
      return TG3_ERR_STREAM_ABORTED;
  }
  tg3__parse_string_array(
    ctx, json_doc, "extensionsUsed", &model->extensions_used, &model->extensions_used_count, 0, "/" );
  tg3__parse_string_array(
    ctx, json_doc, "extensionsRequired", &model->extensions_required, &model->extensions_required_count, 0, "/" );
  model->default_scene = -1;
  ...
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "accessors", tg3_accessor, model->accessors, model->accessors_count, tg3__parse_accessor );
  ...
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "materials", tg3_material, model->materials, model->materials_count, tg3__parse_material );
  TG3__STREAM_CB( on_material, model->materials, model->materials_count );
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "textures", tg3_texture, model->textures, model->textures_count, tg3__parse_texture );
  TG3__STREAM_CB( on_texture, model->textures, model->textures_count );
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "samplers", tg3_sampler, model->samplers, model->samplers_count, tg3__parse_sampler );
  ...
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "animations", tg3_animation, model->animations, model->animations_count, tg3__parse_animation );
  TG3__STREAM_CB( on_animation, model->animations, model->animations_count );
  TG3__PARSE_ARRAY_SIMPLE(
    ctx, json_doc, "cameras", tg3_camera, model->cameras, model->cameras_count, tg3__parse_camera );
  ...
  if ( tg3__json_is_object( ext_it ) )
  {
    ...
    {
      TG3__PARSE_ARRAY_SIMPLE(
        ctx, lights_ext, "lights", tg3_light, model->lights, model->lights_count, tg3__parse_light );
    ...
    if ( tg3__json_is_object( audio_ext ) )
    {
      TG3__PARSE_ARRAY_SIMPLE( ctx,
                               ...
                               tg3_audio_source,
                               model->audio_sources,
                               model->audio_sources_count,
                               tg3__parse_audio_source );
      TG3__PARSE_ARRAY_SIMPLE( ctx,
                               audio_ext,
                               "emitters",
                               tg3_audio_emitter,
                               model->audio_emitters,
                               model->audio_emitters_count,
...
static void tg3__json_parse_options_from_tg3( const tg3_parse_options* options, tg3json_parse_options* json_options )
{
...
static tg3_error_code tg3__parse_glb_header( const uint8_t* data,
                                             uint64_t size,
                                             ...
                                             tg3_error_stack* errors )
{
  ...
  *bin_size_out = 0;
  ...
  {
    tg3__error_push(
      errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_INVALID_HEADER, "GLB data too small for header", NULL, -1 );
    return TG3_ERR_GLB_INVALID_HEADER;
  }
  if ( data[0] != 'g' || data[1] != 'l' || data[2] != 'T' || data[3] != 'F' )
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_INVALID_MAGIC, "Invalid GLB magic bytes", NULL, -1 );
  ...
  {
    tg3__error_push(
      errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_INVALID_VERSION, "Unsupported GLB version (expected 2)", NULL, -1 );
  ...
  {
    tg3__error_push(
      errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_SIZE_MISMATCH, "GLB total length does not match data size", NULL, -1 );
  ...
  while ( offset + 8 <= (uint64_t)total_length )
  {
    ...
    uint32_t chunk_type;
    TINYGLTF3_MEMCPY( &chunk_length, data + offset, 4 );
    offset += 4;
    TINYGLTF3_MEMCPY( &chunk_type, data + offset, 4 );
    offset += 4;
    if ( offset + chunk_length > (uint64_t)total_length )
    {
      tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_CHUNK_ERROR, "GLB chunk exceeds data size", NULL, -1 );
    ...
    offset += chunk_length;
    while ( ( offset & 3u ) != 0u && offset < size )
      ++offset;
  }
  if ( !*json_out )
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_GLB_CHUNK_ERROR, "GLB missing JSON chunk", NULL, -1 );
    return TG3_ERR_GLB_CHUNK_ERROR;
  }
  return TG3_OK;
}

TINYGLTF3_API tg3_error_code tg3_parse( tg3_model* model,
                                        tg3_error_stack* errors,
                                        ...
                                        const tg3_parse_options* options )
{
  ...
  parsed_ok = tg3json_parse_n_opts( (const char*)json_data, (size_t)json_size, &json_options, &json_doc, &error_pos );
  if ( !parsed_ok || json_doc.type != TG3JSON_OBJECT )
  {
    tg3__error_push( errors,
                     TG3_SEVERITY_ERROR,
                     TG3_ERR_JSON_PARSE,
...
TINYGLTF3_API tg3_error_code tg3_parse_glb( tg3_model* model,
                                            tg3_error_stack* errors,
                                            ...
                                            const tg3_parse_options* options )
{
  ...
  tg3__json_parse_options_from_tg3( options, &json_options );
  parsed_ok =
    tg3json_parse_n_opts( (const char*)json_chunk, (size_t)json_chunk_size, &json_options, &json_doc, &error_pos );
  if ( !parsed_ok || json_doc.type != TG3JSON_OBJECT )
  {
    tg3__error_push( errors,
                     TG3_SEVERITY_ERROR,
...
TINYGLTF3_API tg3_error_code tg3_parse_auto( tg3_model* model,
                                             tg3_error_stack* errors,
                                             const uint8_t* data,
                                             uint64_t size,
                                             const char* base_dir,
...
TINYGLTF3_API tg3_error_code tg3_parse_file( tg3_model* model,
                                             tg3_error_stack* errors,
                                             const char* filename,
                                             ...
                                             const tg3_parse_options* options )
{
  ...
  if ( !opts.fs.read_file )
  {
    tg3__error_push( errors,
                     ...
                     "No filesystem callbacks. Define TINYGLTF3_ENABLE_FS or provide fs callbacks.",
                     NULL,
                     -1 );
  ...
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_FILE_NOT_FOUND, "Failed to read file", NULL, -1 );
    return TG3_ERR_FILE_NOT_FOUND;
  }
...
{
  tg3json_value tmp;
  if ( !tg3json_value_init_string_n( &tmp, str, len ) )
    return 0;
  return tg3__json_set_take( obj, key, &tmp );
}
...
static int tg3__json_ensure_object( tg3json_value* obj, const char* key, tg3json_value** out_child )
{
  ...
  if ( existing )
  {
    if ( existing->type != TG3JSON_OBJECT )
      return 0;
    *out_child = existing;
    return 1;
  }
  {
    ...
    if ( !tg3__json_set_take( obj, key, &tmp ) )
      return 0;
  }
  existing = tg3json_object_get_mut( obj, key );
  if ( !existing || existing->type != TG3JSON_OBJECT )
    return 0;
  *out_child = existing;
  return 1;
}
...
static int tg3__json_ensure_array( tg3json_value* obj, const char* key, tg3json_value** out_child )
{
  ...
  if ( existing )
  {
    if ( existing->type != TG3JSON_ARRAY )
      return 0;
    *out_child = existing;
    return 1;
  }
  ...
  existing = tg3json_object_get_mut( obj, key );
  if ( !existing || existing->type != TG3JSON_ARRAY )
    return 0;
  *out_child = existing;
  return 1;
}
...
static int tg3__serialize_int( tg3json_value* o, const char* key, int32_t val, int32_t default_val, int write_defaults )
{
...
static int tg3__serialize_uint64(
  tg3json_value* o, const char* key, uint64_t val, uint64_t default_val, int write_defaults )
{
  if ( val == default_val && !write_defaults )
    return 1;
...
static int tg3__serialize_double(
  tg3json_value* o, const char* key, double val, double default_val, int write_defaults )
{
  if ( tg3__fabs( val - default_val ) <= 1e-12 && !write_defaults )
    return 1;
  return tg3__json_set_real( o, key, val );
}

static int tg3__serialize_bool(
  tg3json_value* o, const char* key, int32_t val, int32_t default_val, int write_defaults )
{
  if ( val == default_val && !write_defaults )
    return 1;
...
static int tg3__serialize_double_array( tg3json_value* o, const char* key, const double* arr, uint32_t count )
{
  tg3json_value tmp;
  if ( !arr || count == 0 )
    return 1;
  if ( !tg3__json_from_double_array( arr, count, &tmp ) )
    return 0;
  return tg3__json_set_take( o, key, &tmp );
}

static int tg3__serialize_int_array( tg3json_value* o, const char* key, const int32_t* arr, uint32_t count )
{
  tg3json_value tmp;
  if ( !arr || count == 0 )
    return 1;
  if ( !tg3__json_from_int_array( arr, count, &tmp ) )
    return 0;
  return tg3__json_set_take( o, key, &tmp );
}

static int tg3__serialize_string_array( tg3json_value* o, const char* key, const tg3_str* arr, uint32_t count )
{
  tg3json_value tmp;
  if ( !arr || count == 0 )
    return 1;
  if ( !tg3__json_from_string_array( arr, count, &tmp ) )
    return 0;
  return tg3__json_set_take( o, key, &tmp );
}
...
static int tg3__value_to_json( const tg3_value* v, tg3json_value* out )
{
  ...
  case TG3_VALUE_STRING:
    return tg3json_value_init_string_n(
      out, v->string_val.data ? v->string_val.data : "", v->string_val.data ? v->string_val.len : 0 );
...
static int tg3__serialize_extras_ext( tg3json_value* o, const tg3_extras_ext* ee )
{
  ...
  if ( ee->extras )
  {
    ...
    if ( !tg3__value_to_json( ee->extras, &extras ) )
      return 0;
    if ( !tg3__json_set_take( o, "extras", &extras ) )
      return 0;
  }
  if ( ee->extensions && ee->extensions_count > 0 )
  {
    tg3json_value* exts = NULL;
    if ( !tg3__json_ensure_object( o, "extensions", &exts ) )
      return 0;
    for ( i = 0; i < ee->extensions_count; ++i )
    {
      tg3json_value item;
      if ( !tg3__value_to_json( &ee->extensions[i].value, &item ) )
        return 0;
...
static int tg3__serialize_texture_info( tg3json_value* parent, const char* key, const tg3_texture_info* ti, int wd )
{
  tg3json_value o;
  if ( ti->index < 0 )
    return 1;
  tg3json_value_init_object( &o );
  if ( !tg3__json_set_int( &o, "index", ti->index ) || !tg3__serialize_int( &o, "texCoord", ti->tex_coord, 0, wd ) ||
       !tg3__serialize_extras_ext( &o, &ti->ext ) || !tg3__json_set_take( parent, key, &o ) )
  {
...
static int tg3__serialize_normal_texture_info( tg3json_value* parent,
                                               const char* key,
                                               ...
                                               int wd )
{
  ...
  tg3json_value_init_object( &o );
  if ( !tg3__json_set_int( &o, "index", ti->index ) || !tg3__serialize_int( &o, "texCoord", ti->tex_coord, 0, wd ) ||
       !tg3__serialize_double( &o, "scale", ti->scale, 1.0, wd ) || !tg3__serialize_extras_ext( &o, &ti->ext ) ||
...
static int tg3__serialize_occlusion_texture_info( tg3json_value* parent,
                                                  const char* key,
                                                  ...
                                                  int wd )
{
  ...
  tg3json_value_init_object( &o );
  if ( !tg3__json_set_int( &o, "index", ti->index ) || !tg3__serialize_int( &o, "texCoord", ti->tex_coord, 0, wd ) ||
       !tg3__serialize_double( &o, "strength", ti->strength, 1.0, wd ) || !tg3__serialize_extras_ext( &o, &ti->ext ) ||
       !tg3__json_set_take( parent, key, &o ) )
  {
    tg3json_value_free( &o );
    return 0;
...
static int tg3__serialize_asset( const tg3_asset* a, int wd, tg3json_value* out )
{
  (void)wd;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "version", a->version ) || !tg3__serialize_str( out, "generator", a->generator ) ||
       !tg3__serialize_str( out, "minVersion", a->min_version ) ||
       !tg3__serialize_str( out, "copyright", a->copyright ) || !tg3__serialize_extras_ext( out, &a->ext ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
static int tg3__serialize_buffer( const tg3_buffer* b, int wd, int embed, tg3json_value* out )
{
  ...
  if ( !tg3__serialize_str( out, "name", b->name ) ||
       !tg3__json_set_int( out, "byteLength", (int64_t)( b->byte_length ? b->byte_length : b->data.count ) ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
{
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", bv->name ) || !tg3__json_set_int( out, "buffer", bv->buffer ) ||
  ...
  if ( !tg3__serialize_int( out, "target", bv->target, 0, wd ) || !tg3__serialize_extras_ext( out, &bv->ext ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
static int tg3__serialize_accessor( const tg3_accessor* acc, int wd, tg3json_value* out )
{
  ...
  if ( acc->sparse.is_sparse )
  {
    ...
    if ( !tg3__json_set_int( &sparse, "count", acc->sparse.count ) ||
         ...
         !tg3__json_set_take( &sparse, "values", &values ) || !tg3__serialize_extras_ext( &sparse, &acc->sparse.ext ) ||
...
static int tg3__serialize_image( const tg3_image* img, int wd, int embed, tg3json_value* out )
{
  ...
  (void)embed;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", img->name ) || !tg3__serialize_str( out, "uri", img->uri ) ||
       !tg3__serialize_str( out, "mimeType", img->mime_type ) || !tg3__serialize_extras_ext( out, &img->ext ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
static int tg3__serialize_texture( const tg3_texture* t, int wd, tg3json_value* out )
{
  (void)wd;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", t->name ) || !tg3__serialize_extras_ext( out, &t->ext ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
static int tg3__serialize_material( const tg3_material* m, int wd, tg3json_value* out )
{
  ...
  if ( !tg3__serialize_str( out, "name", m->name ) )
    goto fail;
  if ( p->base_color_factor[0] != 1.0 || p->base_color_factor[1] != 1.0 || p->base_color_factor[2] != 1.0 ||
       p->base_color_factor[3] != 1.0 || wd )
  {
    if ( !tg3__serialize_double_array( &pbr, "baseColorFactor", p->base_color_factor, 4 ) )
      goto fail;
    has_pbr = 1;
  }
  if ( !tg3__serialize_double( &pbr, "metallicFactor", p->metallic_factor, 1.0, wd ) )
    goto fail;
  if ( tg3__fabs( p->metallic_factor - 1.0 ) > 1e-12 || wd )
    has_pbr = 1;
  if ( !tg3__serialize_double( &pbr, "roughnessFactor", p->roughness_factor, 1.0, wd ) )
    goto fail;
  if ( tg3__fabs( p->roughness_factor - 1.0 ) > 1e-12 || wd )
    has_pbr = 1;
  if ( p->base_color_texture.index >= 0 )
  {
    if ( !tg3__serialize_texture_info( &pbr, "baseColorTexture", &p->base_color_texture, wd ) )
      goto fail;
    has_pbr = 1;
  }
  if ( p->metallic_roughness_texture.index >= 0 )
  {
    if ( !tg3__serialize_texture_info( &pbr, "metallicRoughnessTexture", &p->metallic_roughness_texture, wd ) )
      goto fail;
    has_pbr = 1;
  }
  if ( !tg3__serialize_extras_ext( &pbr, &p->ext ) )
    goto fail;
  if ( ( has_pbr || wd ) && !tg3__json_set_take( out, "pbrMetallicRoughness", &pbr ) )
    goto fail;
  if ( !tg3__serialize_normal_texture_info( out, "normalTexture", &m->normal_texture, wd ) ||
       ...
       !tg3__serialize_texture_info( out, "emissiveTexture", &m->emissive_texture, wd ) )
    goto fail;
  if ( ( m->emissive_factor[0] != 0.0 || m->emissive_factor[1] != 0.0 || m->emissive_factor[2] != 0.0 || wd ) &&
       !tg3__serialize_double_array( out, "emissiveFactor", m->emissive_factor, 3 ) )
    goto fail;
  if ( m->alpha_mode.data && !tg3_str_equals_cstr( m->alpha_mode, "OPAQUE" ) &&
       !tg3__serialize_str( out, "alphaMode", m->alpha_mode ) )
    goto fail;
  if ( !tg3__serialize_double( out, "alphaCutoff", m->alpha_cutoff, 0.5, wd ) ||
       !tg3__serialize_bool( out, "doubleSided", m->double_sided, 0, wd ) ||
       !tg3__serialize_extras_ext( out, &m->ext ) )
    goto fail;
  return 1;
fail:
...
static int tg3__serialize_primitive( const tg3_primitive* p, int wd, tg3json_value* out )
{
  ...
  if ( p->indices >= 0 && !tg3__json_set_int( out, "indices", p->indices ) )
    goto fail;
  if ( p->material >= 0 && !tg3__json_set_int( out, "material", p->material ) )
    goto fail;
  if ( !tg3__serialize_int( out, "mode", p->mode, TG3_MODE_TRIANGLES, wd ) )
    goto fail;
  ...
  if ( !tg3__serialize_extras_ext( out, &p->ext ) )
    goto fail;
  return 1;
fail:
  tg3json_value_free( out );
...
static int tg3__serialize_mesh( const tg3_mesh* m, int wd, tg3json_value* out )
{
  ...
  if ( m->primitives && m->primitives_count > 0 )
  {
    ...
    {
      tg3json_value prim;
      if ( !tg3__serialize_primitive( &m->primitives[i], wd, &prim ) || !tg3__json_push_take( &prims, &prim ) )
      {
...
static int tg3__serialize_node( const tg3_node* n, int wd, tg3json_value* out )
{
  ...
  if ( !tg3__serialize_str( out, "name", n->name ) ||
       !tg3__serialize_int_array( out, "children", n->children, n->children_count ) )
    goto fail;
  if ( n->camera >= 0 && !tg3__json_set_int( out, "camera", n->camera ) )
    goto fail;
  if ( n->skin >= 0 && !tg3__json_set_int( out, "skin", n->skin ) )
    goto fail;
  if ( n->mesh >= 0 && !tg3__json_set_int( out, "mesh", n->mesh ) )
    goto fail;
  if ( n->has_matrix )
  {
    if ( !tg3__serialize_double_array( out, "matrix", n->matrix, 16 ) )
      goto fail;
  }
  else
  {
    ...
    if ( ( has_t || wd ) && !tg3__serialize_double_array( out, "translation", n->translation, 3 ) )
      goto fail;
    if ( ( has_r || wd ) && !tg3__serialize_double_array( out, "rotation", n->rotation, 4 ) )
      goto fail;
    if ( ( has_s || wd ) && !tg3__serialize_double_array( out, "scale", n->scale, 3 ) )
      goto fail;
  }
  if ( !tg3__serialize_double_array( out, "weights", n->weights, n->weights_count ) ||
       !tg3__serialize_extras_ext( out, &n->ext ) )
    goto fail;
  if ( n->light >= 0 || n->emitter >= 0 || ( n->lods && n->lods_count > 0 ) )
  {
    if ( !tg3__json_ensure_object( out, "extensions", &exts ) )
      goto fail;
    if ( n->light >= 0 )
    {
      tg3json_value_init_object( &tmp );
      if ( !tg3__json_set_int( &tmp, "light", n->light ) || !tg3__json_set_take( exts, "KHR_lights_punctual", &tmp ) )
      {
    ...
    {
      tg3json_value_init_object( &tmp );
      if ( !tg3__json_set_int( &tmp, "emitter", n->emitter ) || !tg3__json_set_take( exts, "KHR_audio", &tmp ) )
      {
        tg3json_value_free( &tmp );
        goto fail;
      }
...
static int tg3__serialize_skin( const tg3_skin* s, int wd, tg3json_value* out )
{
  ...
  if ( s->inverse_bind_matrices >= 0 && !tg3__json_set_int( out, "inverseBindMatrices", s->inverse_bind_matrices ) )
  {
    tg3json_value_free( out );
    return 0;
  }
...
static int tg3__serialize_animation( const tg3_animation* a, int wd, tg3json_value* out )
{
  ...
  (void)wd;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", a->name ) )
    goto fail;
  if ( a->channels && a->channels_count > 0 )
  {
    ...
    for ( i = 0; i < a->channels_count; ++i )
    {
      ...
      tg3json_value_init_object( &tgt );
      if ( a->channels[i].target.node >= 0 && !tg3__json_set_int( &tgt, "node", a->channels[i].target.node ) )
      {
        tg3json_value_free( &tgt );
      ...
      if ( !tg3__json_set_int( &ch, "sampler", a->channels[i].sampler ) ||
           ...
           !tg3__json_set_take( &ch, "target", &tgt ) || !tg3__serialize_extras_ext( &ch, &a->channels[i].ext ) ||
  ...
  if ( a->samplers && a->samplers_count > 0 )
  {
    ...
    for ( i = 0; i < a->samplers_count; ++i )
    {
      ...
      if ( !tg3__json_set_int( &s, "input", a->samplers[i].input ) ||
           ...
           !tg3__serialize_extras_ext( &s, &a->samplers[i].ext ) || !tg3__json_push_take( &samplers, &s ) )
      {
  ...
  if ( !tg3__serialize_extras_ext( out, &a->ext ) )
    goto fail;
  return 1;
fail:
  tg3json_value_free( out );
...
static int tg3__serialize_camera( const tg3_camera* c, int wd, tg3json_value* out )
{
  ...
  (void)wd;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", c->name ) || !tg3__serialize_str( out, "type", c->type ) )
    goto fail;
  if ( c->type.data && tg3_str_equals_cstr( c->type, "perspective" ) )
  {
    ...
    if ( ( c->perspective.aspect_ratio > 0.0 &&
         ...
         ( c->perspective.zfar > 0.0 && !tg3__json_set_real( &tmp, "zfar", c->perspective.zfar ) ) ||
         !tg3__json_set_real( &tmp, "znear", c->perspective.znear ) ||
         !tg3__serialize_extras_ext( &tmp, &c->perspective.ext ) || !tg3__json_set_take( out, "perspective", &tmp ) )
    {
      tg3json_value_free( &tmp );
      goto fail;
    }
  ...
  else if ( c->type.data && tg3_str_equals_cstr( c->type, "orthographic" ) )
  {
    ...
    if ( !tg3__json_set_real( &tmp, "xmag", c->orthographic.xmag ) ||
         ...
         !tg3__serialize_extras_ext( &tmp, &c->orthographic.ext ) || !tg3__json_set_take( out, "orthographic", &tmp ) )
    {
      tg3json_value_free( &tmp );
      goto fail;
    }
  }
  if ( !tg3__serialize_extras_ext( out, &c->ext ) )
    goto fail;
  return 1;
fail:
  tg3json_value_free( out );
...
static int tg3__serialize_light( const tg3_light* l, int wd, tg3json_value* out )
{
  tg3json_value spot;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_str( out, "name", l->name ) || !tg3__serialize_str( out, "type", l->type ) ||
       !tg3__serialize_double( out, "intensity", l->intensity, 1.0, wd ) ||
       !tg3__serialize_double( out, "range", l->range, 0.0, wd ) )
    goto fail;
  if ( ( l->color[0] != 1.0 || l->color[1] != 1.0 || l->color[2] != 1.0 || wd ) &&
       !tg3__serialize_double_array( out, "color", l->color, 3 ) )
    goto fail;
  if ( l->type.data && tg3_str_equals_cstr( l->type, "spot" ) )
  {
    ...
    if ( !tg3__serialize_double( &spot, "innerConeAngle", l->spot.inner_cone_angle, 0.0, wd ) ||
         ...
         !tg3__serialize_extras_ext( &spot, &l->spot.ext ) || !tg3__json_set_take( out, "spot", &spot ) )
    {
      tg3json_value_free( &spot );
      goto fail;
    }
  }
  if ( !tg3__serialize_extras_ext( out, &l->ext ) )
    goto fail;
  return 1;
fail:
  tg3json_value_free( out );
...
static int tg3__serialize_model(
  const tg3_model* model, int wd, int embed_images, int embed_buffers, tg3json_value* out )
{
  tg3json_value asset, arr, item, lights_ext, lights, *exts;
  uint32_t i;
  tg3json_value_init_object( out );
  if ( !tg3__serialize_asset( &model->asset, wd, &asset ) || !tg3__json_set_take( out, "asset", &asset ) )
    goto fail;
  if ( model->default_scene >= 0 && !tg3__json_set_int( out, "scene", model->default_scene ) )
    goto fail;
  if ( !tg3__serialize_string_array( out, "extensionsUsed", model->extensions_used, model->extensions_used_count ) ||
       !tg3__serialize_string_array(
         out, "extensionsRequired", model->extensions_required, model->extensions_required_count ) )
    goto fail;
...
#define TG3__SERIALIZE_ARRAY_FIELD( json_key, arr_ptr, arr_count, fn, ... )                                            \
  ...
  TG3__SERIALIZE_ARRAY_FIELD(
    "buffers", model->buffers, model->buffers_count, tg3__serialize_buffer, wd, embed_buffers );
  TG3__SERIALIZE_ARRAY_FIELD(
    "bufferViews", model->buffer_views, model->buffer_views_count, tg3__serialize_buffer_view, wd );
...
#undef TG3__SERIALIZE_ARRAY_FIELD
  ...
  if ( model->lights && model->lights_count > 0 )
  {
    ...
    {
      tg3json_value_init_null( &item );
      if ( !tg3__serialize_light( &model->lights[i], wd, &item ) || !tg3__json_push_take( &lights, &item ) )
      {
        tg3json_value_free( &item );
  ...
  if ( !tg3__serialize_extras_ext( out, &model->ext ) )
    goto fail;
  return 1;
fail:
  tg3json_value_free( out );
...
TINYGLTF3_API tg3_error_code tg3_write_to_memory( const tg3_model* model,
                                                  tg3_error_stack* errors,
                                                  uint8_t** out_data,
                                                  uint64_t* out_size,
                                                  const tg3_write_options* options )
{
  ...
  if ( !tg3__serialize_model(
         model, options->serialize_defaults, options->embed_images, options->embed_buffers, &root ) )
  {
    tg3__error_push(
      errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "Failed to serialize model to JSON tree", NULL, -1 );
    return TG3_ERR_OUT_OF_MEMORY;
  }
  json_str =
    options->pretty_print ? tg3json_stringify_pretty( &root, 2, &json_len ) : tg3json_stringify( &root, &json_len );
  tg3json_value_free( &root );
  if ( !json_str )
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "Failed to stringify JSON output", NULL, -1 );
    return TG3_ERR_OUT_OF_MEMORY;
  }
  if ( options->write_binary )
  {
    ...
    if ( !glb )
    {
      ...
      tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM allocating GLB output", NULL, -1 );
      return TG3_ERR_OUT_OF_MEMORY;
    }
    ...
    if ( bin_data && bin_len > 0 )
    {
      ...
      for ( i = (uint32_t)bin_len; i < bin_padded; ++i )
        glb[bin_off + 8u + i] = 0;
    }
    TINYGLTF3_FREE( json_str );
    *out_data = glb;
  ...
  else
  {
    ...
    {
      TINYGLTF3_FREE( json_str );
      tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_OUT_OF_MEMORY, "OOM allocating JSON output", NULL, -1 );
      return TG3_ERR_OUT_OF_MEMORY;
    }
...
TINYGLTF3_API tg3_error_code tg3_write_to_file( const tg3_model* model,
                                                tg3_error_stack* errors,
                                                const char* filename,
                                                uint32_t filename_len,
                                                const tg3_write_options* options )
{
  ...
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_FS_NOT_AVAILABLE, "No filesystem write callback", NULL, -1 );
    return TG3_ERR_FS_NOT_AVAILABLE;
  }
  ...
  if ( !ok )
  {
    tg3__error_push( errors, TG3_SEVERITY_ERROR, TG3_ERR_FILE_WRITE, "Failed to write file", NULL, -1 );
...
TINYGLTF3_API tg3_writer* tg3_writer_create( tg3_write_chunk_fn chunk_fn,
                                             void* user_data,
                                             const tg3_write_options* options )
{
  ...
  if ( options )
    ...
    tg3_write_options_init( &w->options );
...
static tg3_error_code tg3__writer_add_item( tg3_writer* w, const char* json_key, const tg3json_value* item )
{
  ...
  if ( !w || !w->begun )
    return TG3_ERR_WRITE_FAILED;
  if ( !tg3__json_ensure_array( &w->root, json_key, &arr ) || !tg3json_array_append_copy( arr, item ) )
  {
    return TG3_ERR_OUT_OF_MEMORY;
  }
  return TG3_OK;
...
#define TG3__WRITER_ADD_IMPL( name, Type, json_key, serialize_fn, ... )                                                \
  ...
  {                                                                                                                    \
    ...
    if ( !item )                                                                                                       \
      return TG3_ERR_WRITE_FAILED;                                                                                     \
    if ( !serialize_fn( item, w->options.serialize_defaults, __VA_ARGS__, &tmp ) )                                     \
      return TG3_ERR_OUT_OF_MEMORY;                                                                                    \
    {                                                                                                                  \
      tg3_error_code err = tg3__writer_add_item( w, json_key, &tmp );                                                  \
      tg3json_value_free( &tmp );                                                                                      \
      return err;                                                                                                      \
    }                                                                                                                  \
  }
...
#define TG3__WRITER_ADD_SIMPLE( name, Type, json_key, serialize_fn )                                                   \
  ...
  {                                                                                                                    \
    ...
    if ( !item )                                                                                                       \
      return TG3_ERR_WRITE_FAILED;                                                                                     \
    if ( !serialize_fn( item, w->options.serialize_defaults, &tmp ) )                                                  \
      return TG3_ERR_OUT_OF_MEMORY;                                                                                    \
    {                                                                                                                  \
      tg3_error_code err = tg3__writer_add_item( w, json_key, &tmp );                                                  \
      tg3json_value_free( &tmp );                                                                                      \
...
TINYGLTF3_API tg3_error_code tg3_writer_end( tg3_writer* w )
{
  ...
  size_t json_len;
  int32_t ok;
  if ( !w || !w->begun || !w->chunk_fn )
    return TG3_ERR_WRITE_FAILED;
  json_str = w->options.pretty_print ? tg3json_stringify_pretty( &w->root, 2, &json_len )
