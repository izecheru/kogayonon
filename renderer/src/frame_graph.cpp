    ...
    for ( Node* node : m_container.executionOrder )
    {
        for ( FGResource* resource : node->reads )
        {
            if ( resource->lastState.accessType == FGResourceAccessType::Write ) // RAW
            {
                ...
                else if ( desiredState.type ==
                          FGResourceType::Depth ) // if we want to read a previously written depth image
                {
                    transitionBarrier =
                ...
                else // if we don't have a color transfer or a previously written detph image, this also covers the
                     // Shader type and the Color case
                {
                    VkImageAspectFlags aspect{ VK_IMAGE_ASPECT_COLOR_BIT };
        ...
        for ( FGResource* resource : node->writes )
        {
            ...
            if ( resource->lastState.accessType == FGResourceAccessType::None ) // Undefined case
            {
                ...
                if ( desiredState.type == FGResourceType::Color &&
                     desiredState.accessType == FGResourceAccessType::Write )
                {
                    transitionBarrier =
                        VkImageMemoryBarrier2{ .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
